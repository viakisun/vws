-- Migration 009: Fix trigger and add salary permission
-- 트리거를 임시 비활성화하고 salary.management 권한 추가
...
SELECT 
  r.name_ko as "역할",
  COUNT(CASE WHEN p.resource = 'salary.management' THEN 1 END) as "급여관리",
  COUNT(CASE WHEN p.resource = 'hr.payslips' THEN 1 END) as "급여명세서"
FROM roles r
