...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    // 예산 일관성 검증
    const validation = BudgetConsistencyValidator.validateBudgetConsistency(
      project,
      budgets,
    );
    ...
    const overallValidation =
      ValidationUtils.createOverallValidation(validationResults);

    logger.log(
      `✅ [예산 일관성 검증] 완료 - ${validation.isValid ? "✅ 통과" : "❌ 실패"}`,
    );
  ...
  try {
    ...
    if (autoFix && !validation.isValid) {
      ...
      await ValidationUtils.pool.query(
        ...
        [totalBudgetFromBudgets, projectId],
      );
