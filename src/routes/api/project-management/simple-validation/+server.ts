...
async function validateProject(projectId: string): Promise<{
  ...
  try {
    ...
    if (personnelResult.hasIssues) {
      ...
      const fixResult = await fixPersonnelCost(
        client,
        projectId,
        personnelResult.issues,
      );
      if (fixResult.success) {
        fixedIssues += fixResult.fixedCount;
        logger.log(
          `✅ [인건비 수정] 완료: ${fixResult.fixedCount}개 연차 수정`,
        );
      } else {
    ...
    if (budgetResult.hasIssues) {
      ...
      const fixResult = await fixBudgetConsistency(
        client,
        projectId,
        budgetResult.issues,
      );
      if (fixResult.success) {
        fixedIssues += fixResult.fixedCount;
        logger.log(
          `✅ [예산 일관성 수정] 완료: ${fixResult.fixedCount}개 이슈 수정`,
        );
      } else {
...
async function validatePersonnelCost(client: any, projectId: string) {
  ...
  for (const budget of budgetResult.rows) {
    ...
    const periodMembers = memberResult.rows.filter((member) => {
      ...
      return (
        memberStartDate <= budgetEndDate && memberEndDate >= budgetStartDate
      );
    });
    ...
    for (const member of periodMembers) {
      ...
      actualPersonnelCost +=
        monthlyAmount * monthsDiff * (participationRate / 100);
    }
  ...
  return {
    ...
    message:
      issues.length > 0
        ? `${issues.length}개 연차에서 인건비 불일치 발견`
        : "인건비 검증 통과",
  };
}
...
async function validateBudgetConsistency(client: any, projectId: string) {
  const projectResult = await client.query(
    "SELECT budget_total FROM projects WHERE id = $1",
    [projectId],
  );
  const budgetResult = await client.query(
    `
