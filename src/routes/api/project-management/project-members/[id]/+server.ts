...
export const PUT: RequestHandler = async ({ params, request }) => {
	try {
		...
		const { role, startDate, endDate, participationRate, contributionType, contractAmount, status } = data;

		// 필수 필드 검증
		if (role === undefined && startDate === undefined && endDate === undefined && 
			participationRate === undefined && contributionType === undefined && 
			contractAmount === undefined && status === undefined) {
			return json({
				success: false,
		...
		if (contractAmount !== undefined) {
			updateFields.push(`contract_amount = $${paramIndex}`);
			updateValues.push(contractAmount);
			paramIndex++;
		}
		...
		// 계약금액이 변경된 경우 월간금액 재계산
		if (contractAmount !== undefined || participationRate !== undefined) {
			const currentMember = existingMember.rows[0];
			let finalContractAmount = contractAmount !== undefined ? contractAmount : currentMember.contract_amount;
			const finalParticipationRate = participationRate !== undefined ? participationRate : currentMember.participation_rate;
			
			// 계약금액이 변경되지 않았지만 참여율이 변경된 경우, 계약서에서 최신 금액 조회
			if (contractAmount === undefined) {
			...
			`);
			const salaryMultiplier = factorResult.rows.length > 0 ? parseFloat(factorResult.rows[0].factor_value) : 1.15;
			
			// 월간 금액 계산: 계약금액 * 급여배수 * (참여율/100)
			const monthlyAmount = Math.round(finalContractAmount * salaryMultiplier * (finalParticipationRate / 100));
