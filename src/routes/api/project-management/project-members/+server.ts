	...
	try {
		...
		if (contractResult.rows.length > 0) {
			...
			if (allContractsResult.rows.length === 0) {
				return json({
					success: false,
					message: '해당 직원의 급여 계약서가 등록되지 않았습니다. 급여 계약서를 먼저 등록해주세요.',
		...
		const monthlyAmount = Math.round(finalContractAmount * salaryMultiplier * (participationRate / 100));
		...
		const result = await query(`
			...
			RETURNING *
		`, [projectId, employeeId, role, startDate, endDate, participationRate, contributionType, finalContractAmount, monthlyAmount, status]);
