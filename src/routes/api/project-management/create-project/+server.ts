...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (data.evidenceSettings.autoGenerate) {
      ...
      logger.log(
        `✅ [7단계] 증빙 항목 자동 생성 완료 - ${evidenceIds.length}개 항목`,
      );
    } else {
    ...
    if (!finalValidation.isValid) {
      ...
      return json(
        { success: false, errors: finalValidation.errors },
        { status: 400 },
      );
    }
...
async function validateProjectData(data: ProjectCreationRequest) {
  ...
  // 연차별 예산 합계 검증
  const totalBudgetFromPeriods = data.annualPeriods.reduce(
    (sum, period) => sum + period.budget,
    0,
  );
  ...
  for (const member of data.members) {
    if (member.participationRate <= 0 || member.participationRate > 100) {
      errors.push(
        `${member.employeeId}의 참여율은 0% 초과 100% 이하여야 합니다.`,
      );
    }
  }
...
async function createProjectBudgets(
  ...
  for (const period of data.annualPeriods) {
    ...
    logger.log(
      `💰 [생성] ${period.periodNumber}차년도 예산 생성 완료 - ID: ${result.rows[0].id}`,
    );
  }
...
async function createProjectMembers(
  ...
  for (const member of data.members) {
    ...
    logger.log(
      `👥 [생성] 참여연구원 ${member.employeeId} 등록 완료 - ID: ${result.rows[0].id}`,
    );
  }
...
async function createEvidenceItems(
  ...
  for (const period of data.annualPeriods) {
    ...
    if (budgetResult.rows.length === 0) {
      logger.log(
        `❌ [생성] ${period.periodNumber}차년도 예산을 찾을 수 없습니다.`,
      );
      continue;
    }
...
async function _runAutoValidationAndFix(projectId: string) {
  try {
    ...
    const response = await fetch(
      `http://localhost:5173/api/project-management/simple-validation`,
      {
        method: "POST",
        headers: {
