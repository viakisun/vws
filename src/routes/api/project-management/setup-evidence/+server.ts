...
export const POST: RequestHandler = async () => {
	try {
		...
		// 인덱스 생성
		await query(`CREATE INDEX IF NOT EXISTS idx_budget_evidence_project_budget_id ON budget_evidence(project_budget_id)`);
		await query(`CREATE INDEX IF NOT EXISTS idx_budget_evidence_evidence_date ON budget_evidence(evidence_date)`);
		await query(`CREATE INDEX IF NOT EXISTS idx_budget_evidence_status ON budget_evidence(status)`);
		await query(`CREATE INDEX IF NOT EXISTS idx_budget_evidence_created_by ON budget_evidence(created_by)`);
