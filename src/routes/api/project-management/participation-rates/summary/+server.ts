...
export const GET: RequestHandler = async () => {
  try {
    ...
    const stats = {
      ...
      overLimitEmployees: summary.filter(
        (emp) => emp.participationStatus === "OVER_LIMIT",
      ).length,
      fullEmployees: summary.filter((emp) => emp.participationStatus === "FULL")
        .length,
      availableEmployees: summary.filter(
        (emp) => emp.participationStatus === "AVAILABLE",
      ).length,
      averageParticipationRate:
        summary.length > 0
          ? Math.round(
              (summary.reduce(
                (sum, emp) => sum + emp.totalParticipationRate,
                0,
              ) /
                summary.length) *
                10,
