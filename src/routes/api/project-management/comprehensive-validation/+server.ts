...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (
      projectId &&
      (validationScope === "all" || validationScope === "project")
    ) {
      ...
      try {
        ...
        for (const budget of budgets) {
          const actualPersonnelCost =
            PersonnelCostValidator.calculateActualPersonnelCost(
              members,
              budget,
            );
        ...
        const budgetValidation =
          BudgetConsistencyValidator.validateBudgetConsistency(
            project,
            budgets,
          );
        validations.push({
        ...
        for (const member of members) {
          const validation =
            EmploymentPeriodValidator.validateMemberEmploymentPeriod(
              member,
              project,
            );
          validations.push({
    ...
    results.summary.total = allResults.reduce(
      (sum, r) => sum + (r.summary?.total || 0),
      0,
    );
    results.summary.valid = allResults.reduce(
      (sum, r) => sum + (r.summary?.valid || 0),
      0,
    );
    results.summary.invalid = allResults.reduce(
  ...
  try {
    ...
    switch (validationType) {
      ...
      case "query-columns":
        ...
        validationResult = SchemaValidator.validateQueryColumns(
          query,
          tableName,
        );
        break;
    ...
    logger.log(
      `✅ [종합 검증] 완료 - ${validationResult.isValid ? "통과" : "실패"}`,
    );
