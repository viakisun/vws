		...
		for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
			...
			try {
				...
				if (employeeResult.rows.length === 0) {
					results.failed++;
					results.errors.push(`행 ${rowNumber}: 사번 ${employeeId}에 해당하는 직원을 찾을 수 없습니다.`);
					continue;
				}
				...
				const allowances = [
					...
					{ id: 'vehicle_maintenance', name: '차량유지', amount: Number(row.getCell(10).value) || 0 },
					{ id: 'annual_leave_allowance', name: '연차수당', amount: Number(row.getCell(11).value) || 0 },
					{ id: 'year_end_settlement', name: '연말정산', amount: Number(row.getCell(12).value) || 0 }
				];

				// 공제사항 데이터 추출
				const deductions = [
					{ id: 'health_insurance', name: '건강보험', amount: Number(row.getCell(13).value) || 0 },
					{ id: 'long_term_care', name: '장기요양보험', amount: Number(row.getCell(14).value) || 0 },
					{ id: 'national_pension', name: '국민연금', amount: Number(row.getCell(15).value) || 0 },
					{ id: 'employment_insurance', name: '고용보험', amount: Number(row.getCell(16).value) || 0 },
				...
				if (existingPayslip.rows.length > 0) {
					...
					await query(
						...
						`,
						[
							employeeDbId, period, payDate, `${year}-${month.padStart(2, '0')}-01`,
							`${year}-${month.padStart(2, '0')}-${lastDay.toString().padStart(2, '0')}`,
							baseSalary, totalPayments, totalDeductions, netSalary, totalPayments,
