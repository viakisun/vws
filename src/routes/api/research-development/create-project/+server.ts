...
async function validateCreatedProject(projectId: string): Promise<ValidationResult> {
  ...
  const projectResult = await query(
    'SELECT id, name, description, status, created_at::text as created_at, updated_at::text as updated_at FROM rd_rd_projects WHERE id = $1',
    [projectId],
  )
  ...
  const memberResult = await query(
    'SELECT id, project_id, employee_id, role, participation_rate, start_date, end_date, created_at::text as created_at, updated_at::text as updated_at FROM project_members WHERE project_id = $1',
    [projectId],
  )
