...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!expenseData.project_id || !expenseData.category_code || !expenseData.amount || !expenseData.requester_id) {
			return error(400, { message: 'Project ID, category code, amount, and requester ID are required' });
		}
