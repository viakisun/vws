...
export const POST: RequestHandler = async () => {
	try {
		...
		const executivePositions = ['대표', '이사', '상무', '대표이사', '연구소장', '기술이사', '상무이사'];
		...
		console.log(`Found ${executiveEmployees.rows.length} executive employees to migrate`);
		...
		for (const employee of executiveEmployees.rows) {
			...
			if (jobTitleResult.rows.length === 0) {
				console.warn(`Job title ${jobTitleName} not found, skipping ${employee.first_name} ${employee.last_name}`);
				continue;
			}
			...
			console.log(`Migrated ${employee.first_name} ${employee.last_name} to executives table`);
		}
