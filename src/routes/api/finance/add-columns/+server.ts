...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    // 1. 의뢰인/수취인 컬럼 추가
    try {
      await query('ALTER TABLE finance_transactions ADD COLUMN IF NOT EXISTS counterparty VARCHAR(255)')
    ...
    // 2. 입금 컬럼 추가
    try {
      await query('ALTER TABLE finance_transactions ADD COLUMN IF NOT EXISTS deposits DECIMAL(15,2) DEFAULT 0')
    ...
    // 3. 출금 컬럼 추가
    try {
      await query('ALTER TABLE finance_transactions ADD COLUMN IF NOT EXISTS withdrawals DECIMAL(15,2) DEFAULT 0')
    ...
    // 4. 거래후잔액 컬럼 추가
    try {
      await query('ALTER TABLE finance_transactions ADD COLUMN IF NOT EXISTS balance DECIMAL(15,2) DEFAULT 0')
