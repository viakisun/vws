...
<PageLayout title="자금 관리" subtitle="기업 자금 흐름 및 재무 현황 통합 관리 시스템">
  {#if store.ui.loading && !isInitialized}
  ...
  {:else}
    ...
    {#if !isInitialized}
      ...
      >
        ...
        >
          {store.ui.loading ? '초기화 중...' : '시스템 초기화'}
        </button>
      </div>
    {:else if activeTab === 'overview'}
      <FinanceOverviewTab
        stats={{
          totalBalance: store.statistics.totalBalance,
          activeAccounts: store.statistics.activeAccountsCount,
          monthlyIncome: store.statistics.monthlyIncome,
          monthlyExpense: store.statistics.monthlyExpense,
          netCashFlow: store.statistics.netCashFlow,
        }}
        actionItems={finance.statistics.actionItems}
