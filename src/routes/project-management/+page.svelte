<script lang="ts">
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          throw new Error(
            data.message || "프로젝트 데이터를 불러오는데 실패했습니다.",
          );
        }
  ...
  function validateProjectData(projectData) {
    ...
    projectData.forEach((project, index) => {
      const projectName =
        project.title || project.code || `프로젝트 ${index + 1}`;
      ...
      if (!isPlanning) {
        if (!project.startDate) {
          issues.push(
            `${projectName}: 진행/완료 상태 프로젝트는 시작일이 필요합니다.`,
          );
        }
        if (!project.endDate) {
          issues.push(
            `${projectName}: 진행/완료 상태 프로젝트는 종료일이 필요합니다.`,
          );
        }
      } else {
        // 기획 단계에서는 경고만 표시
        if (!project.startDate) {
          warnings.push(
            `${projectName}: 시작일이 설정되지 않았습니다. (기획 단계이므로 선택사항)`,
          );
        }
        if (!project.endDate) {
          warnings.push(
            `${projectName}: 종료일이 설정되지 않았습니다. (기획 단계이므로 선택사항)`,
          );
        }
      }
      ...
      if (project.status && !validStatuses.includes(project.status)) {
        issues.push(
          `${projectName}: 유효하지 않은 상태값입니다. (${project.status})`,
        );
      }
      ...
      if (project.priority) {
        ...
        if (!validPriorities.includes(project.priority)) {
          issues.push(
            `${projectName}: 유효하지 않은 우선순위값입니다. (${project.priority})`,
          );
        }
      }
    });
