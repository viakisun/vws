<script lang="ts">
	...
	interface Rule { id: string; category: '인건비' | '재료비' | '연구활동비' | '여비' | '보고'; title: string; status: CStatus; note?: string }

	const projectId = page.params.projectId as string;
	let rules: Rule[] = [
		{ id: `${projectId}-C1`, category: '인건비', title: '급여명세서 보관', status: '충족' },
		{ id: `${projectId}-C2`, category: '여비', title: '출장보고서 첨부', status: '미비', note: '보고서 누락' },
...
<Card>
	<div class="mb-3 grid grid-cols-1 sm:grid-cols-4 gap-2 text-sm">
		<input class="rounded-md border border-gray-200 px-2 py-1" placeholder="규정 검색" bind:value={query} />
