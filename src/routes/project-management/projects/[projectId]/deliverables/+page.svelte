<script lang="ts">
	...
	interface Deliverable { id: string; title: string; status: DStatus; due: string; owner: string }

	const projectId = page.params.projectId as string;
	// mock per-project deliverables
	let items: Deliverable[] = [
		{ id: `${projectId}-D1`, title: '중간보고서', status: '진행', due: '2025-11-15', owner: '김철수' },
		{ id: `${projectId}-D2`, title: '프로토타입 v1', status: '지연', due: '2025-12-01', owner: '이영희' },
...
<Card>
	<div class="mb-3 flex flex-col sm:flex-row gap-2 sm:items-center">
		<input class="w-full sm:w-64 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="항목 검색" bind:value={query} />
