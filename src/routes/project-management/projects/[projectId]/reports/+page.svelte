<script lang="ts">
	...
	let items: Report[] = [
		{ id: `${projectId}-R1`, title: '9월 월간 진도보고', status: '제출', period: '2025-09', submittedAt: '2025-10-02' },
	...
	function exportCSV() {
		...
		const rows = filtered.map((r) => `${r.id},${r.title},${r.status},${r.period},${r.submittedAt ?? ''}`).join('\n');
...
<Card>
	<div class="mb-3 flex flex-col sm:flex-row gap-2 sm:items-center">
		<input class="w-full sm:w-64 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="보고서 검색" bind:value={query} />
