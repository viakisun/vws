...
<div class="mb-3">
  <input class="w-full sm:w-72 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="프로젝트 검색 (이름/ID)" bind:value={query} />
...
<div class="space-y-3">
  {#each projects as p}
    <Card>
      <a class="flex items-center justify-between gap-4 hover:bg-gray-50 rounded-md px-2 py-2 transition-colors" href={`/project-management/projects/${p.id}`}>
        <div>
        ...
        <Badge color={p.status === '지연' ? 'yellow' : p.status === '위험' ? 'red' : p.status === '진행중' ? 'blue' : 'green'}>{p.status}</Badge>
      </a>
    </Card>
  {/each}
</div>
