...
<div class="space-y-6">
	...
	<Card header="프로젝트 현황">
		<div class="mb-3 flex flex-col sm:flex-row gap-2 sm:items-center">
			<input class="w-full sm:w-64 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="프로젝트 검색" bind:value={query} />
		...
		<div class="divide-y">
			{#each filtered as p}
				<button type="button" class="w-full text-left flex items-center justify-between gap-4 py-3 hover:bg-gray-50 rounded-md px-2" onclick={() => openDetail(p.id)}>
					<div>
					...
					<Badge color={p.status === '지연' ? 'yellow' : p.status === '진행중' ? 'blue' : p.status === '위험' ? 'red' : 'green'}>{p.status}</Badge>
				</button>
			{/each}
		</div>
	</Card>
		...
		{#if selected}
			<div class="space-y-4">
				<div class="flex items-center justify-between">
					<Badge color={selected.status === '지연' ? 'yellow' : selected.status === '진행중' ? 'blue' : selected.status === '위험' ? 'red' : 'green'}>{selected.status}</Badge>
					...
					{:else}
						<ul class="list-disc pl-5 text-sm space-y-1">
							{#each selected.risks as r}
								<li>
									<span class="font-medium">[{r.severity}]</span> {r.description} <span class="text-caption">({r.impact}/{r.status})</span>
								</li>
							{/each}
						</ul>
					{/if}
				...
				<div>
					...
					<div class="text-sm">참여 인원 {selectedMembers.length}명 · 월 추정 {formatKRW(selectedCostMonthly)}</div>
					<div class="mt-2">
						<a class="text-primary hover:underline" href={`/personnel?projectId=${selected.id}`}>상세 보기 (인건비 관리로 이동)</a>
					</div>
				</div>
			</div>
		{/if}
