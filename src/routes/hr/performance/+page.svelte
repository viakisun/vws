<script lang="ts">
	...
	let reviewForm = $state({
		...
		goals: [{ id: '', title: '', description: '', target: '', actual: '', achievement: 0, weight: 0, rating: 0, comments: '' }],
	...
	let employeeReviews = $derived(getPerformanceReviewsByEmployee(selectedEmployeeId, $performanceReviews));
	let employeeFeedback = $derived(getFeedback360ByReviewee(selectedEmployeeId, $feedback360));
	let employeeCompetencies = $derived(getCompetencyMatrixByEmployee(selectedEmployeeId, $competencyMatrix));
	let employeeTraining = $derived(getTrainingRecordsByEmployee(selectedEmployeeId, $trainingRecords));
	...
	function getStatusBadgeVariant(status: PerformanceReview['status']): 'secondary' | 'warning' | 'success' | 'primary' {
		switch (status) {
					...
					<div class="p-6">
						...
						<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
							...
							<div class="text-center">
								...
								<p class="text-sm text-yellow-500">{getRatingStars(averageFeedback.communication)}</p>
							</div>
							<div class="text-center">
							...
							<div class="text-center">
								...
								<p class="text-sm text-yellow-500">{getRatingStars(averageFeedback.technicalSkills)}</p>
							</div>
						</div>
					</div>
				</Card>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
								...
								<div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
									<div>
										<p class="text-sm font-medium text-gray-900">
											{review.reviewType === 'annual' ? '연간' : review.reviewType === 'quarterly' ? '분기' : '프로젝트'} 평가
										</p>
				...
				<Card>
					<div class="p-6">
						...
						<div class="space-y-3">
							{#each employeeFeedback.filter(feedback => 
								new Date(feedback.reviewPeriod.startDate).getFullYear() === selectedYear
							) as feedback}
								<div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
									<div>
										<p class="text-sm font-medium text-gray-900">
											{feedback.reviewerType === 'manager' ? '상사' : 
											 feedback.reviewerType === 'peer' ? '동료' : 
											 feedback.reviewerType === 'subordinate' ? '부하직원' : '자기평가'}
										</p>
										<p class="text-xs text-gray-500">
											종합점수: {feedback.leadership + feedback.communication + feedback.teamwork + feedback.problemSolving + feedback.initiative + feedback.adaptability + feedback.technicalSkills}/35
										</p>
									</div>
									<div class="flex items-center space-x-2">
							...
							{#each employeeCompetencies as competency}
								<div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
									<div>
										...
										<p class="text-xs text-gray-500">마지막 평가: {formatDate(competency.lastAssessed)}</p>
									</div>
									<div class="flex items-center space-x-2">
										<div class="flex">
											{#each Array(5) as _, i}
												<div class="w-4 h-4 {i < competency.level ? 'bg-blue-500' : 'bg-gray-300'} rounded-full mr-1"></div>
											{/each}
										</div>
						...
						<div class="space-y-3">
							{#each employeeTraining.slice(0, 5) as training}
								<div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
									...
									<div class="flex items-center space-x-2">
										<Badge variant={training.status === 'completed' ? 'success' : training.status === 'in-progress' ? 'warning' : 'secondary'}>
											{training.status === 'completed' ? '완료' : training.status === 'in-progress' ? '진행중' : '예정'}
										</Badge>
										{#if training.score}
