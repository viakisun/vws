<script lang="ts">
	...
	function getStatusBadgeVariant(status: Employee['status']): 'success' | 'warning' | 'danger' | 'secondary' {
		switch (status) {
			...
			<div class="overflow-x-auto">
				<table class="min-w-full divide-y divide-gray-200">
					<thead class="bg-gray-50">
						<tr>
							<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick={() => sort('employeeId')}>
								사번 {#if sortBy === 'employeeId'}{sortOrder === 'asc' ? '↑' : '↓'}{/if}
							</th>
							<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick={() => sort('name')}>
								이름 {#if sortBy === 'name'}{sortOrder === 'asc' ? '↑' : '↓'}{/if}
							</th>
							<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick={() => sort('department')}>
							...
							<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick={() => sort('hireDate')}>
						...
						<div>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div>
									...
									<p class="text-sm text-gray-900">{formatDate(selectedEmployee.personalInfo.birthDate)}</p>
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-500">성별</label>
									<p class="text-sm text-gray-900">{selectedEmployee.personalInfo.gender === 'male' ? '남성' : selectedEmployee.personalInfo.gender === 'female' ? '여성' : '기타'}</p>
								</div>
								<div>
								...
								<div>
									...
									<p class="text-sm text-gray-900">
										{selectedEmployee.personalInfo.maritalStatus === 'single' ? '미혼' : 
										 selectedEmployee.personalInfo.maritalStatus === 'married' ? '기혼' : 
										 selectedEmployee.personalInfo.maritalStatus === 'divorced' ? '이혼' : '사별'}
									</p>
								</div>
							</div>
						</div>
							...
							<div>
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<div>
										...
										<p class="text-sm text-gray-900">{selectedContract.salary.toLocaleString()}원</p>
									</div>
									<div>
