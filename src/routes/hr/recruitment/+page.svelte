<script lang="ts">
	...
	let candidateForm = $state({
		...
		skills: [''],
		languages: [{ language: '', proficiency: 'intermediate' as Candidate['languages'][0]['proficiency'] }]
	});
	...
	function getStatusBadgeVariant(status: JobPosting['status']): 'secondary' | 'success' | 'danger' | 'warning' {
		switch (status) {
	...
	<div class="max-w-7xl mx-auto">
			...
			<div class="p-6">
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					...
					<div class="flex items-end">
						<button
							onclick={() => { statusFilter = ''; departmentFilter = ''; }}
					...
					<div class="p-6">
						...
						<div class="space-y-2 mb-4">
							<p class="text-sm text-gray-600">
								<strong>급여:</strong> {jobPosting.salaryRange.min.toLocaleString()}원 - {jobPosting.salaryRange.max.toLocaleString()}원
							</p>
							<p class="text-sm text-gray-600">
								<strong>위치:</strong> {jobPosting.location}
							</p>
							<p class="text-sm text-gray-600">
								<strong>마감일:</strong> {formatDate(jobPosting.applicationDeadline)}
							</p>
						</div>
		...
		<Modal bind:open={isJobPostingModalOpen}>
			<div class="p-6 max-w-4xl">
				...
				<form onsubmit={(e) => { e.preventDefault(); handleJobPostingSubmit(); }}>
					<div class="space-y-6">
