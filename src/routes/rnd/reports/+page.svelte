...
<div class="container mx-auto p-6">
	<div class="mb-6">
		...
		<p class="text-gray-600">프로젝트별 주간/분기 진도보고서를 생성, 관리하고 자동화된 리포트를 확인합니다.</p>
	</div>
				...
				<div class="flex justify-between items-start mb-4">
					<div class="flex-1">
						...
						<div class="text-sm text-gray-600 mb-3">
							<span class="font-medium">프로젝트:</span> {getProjectName(report.projectId)} | 
							<span class="font-medium">기간:</span> {formatDate(report.periodStart)} ~ {formatDate(report.periodEnd)} | 
							<span class="font-medium">생성자:</span> {getPersonName(report.generatedBy)} | 
							<span class="font-medium">생성일:</span> {formatDate(report.generatedAt)}
						</div>
					</div>
			...
			<div>
				...
				<div class="grid grid-cols-2 gap-4 text-sm">
					...
					<div>
						...
						<span class="ml-2">{formatDate(selectedReport.periodStart)} ~ {formatDate(selectedReport.periodEnd)}</span>
					</div>
					<div>
...
<Modal bind:open={showGenerateModal} title="리포트 생성">
	<div class="space-y-4">
		<div class="grid grid-cols-2 gap-4">
			<div>
				<label for="generate-project" class="block text-sm font-medium text-gray-700 mb-1">프로젝트 *</label>
				<select
					id="generate-project"
			...
			<div>
				<label for="generate-type" class="block text-sm font-medium text-gray-700 mb-1">리포트 타입 *</label>
				<select
					id="generate-type"
		...
		<div class="grid grid-cols-2 gap-4">
			<div>
				<label for="generate-start" class="block text-sm font-medium text-gray-700 mb-1">시작일 *</label>
				<input
					id="generate-start"
					type="date"
			...
			<div>
				<label for="generate-end" class="block text-sm font-medium text-gray-700 mb-1">종료일 *</label>
				<input
					id="generate-end"
					type="date"
