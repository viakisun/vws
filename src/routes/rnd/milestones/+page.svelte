<script lang="ts">
	...
	function createMilestone() {
		if (!formData.projectId || !formData.quarter || !formData.title || !formData.ownerId || !formData.dueDate) {
	...
	function updateMilestoneStatus(milestoneId: string, status: 'pending' | 'in_progress' | 'completed') {
...
<div class="container mx-auto p-6">
	...
	<div class="bg-white rounded-lg shadow-sm border p-4 mb-6">
		<div class="grid grid-cols-1 md:grid-cols-6 gap-4">
			...
			<div>
				<label for="rnd-mil-status-filter" class="block text-sm font-medium text-gray-700 mb-1">상태</label>
				<select
				...
				<div class="flex justify-between items-start mb-4">
					<div class="flex-1">
						<div class="flex items-center gap-3 mb-2">
							...
							<Badge variant={getStatusVariant(milestone.status)}>{getStatusText(milestone.status)}</Badge>
						</div>
						<p class="text-gray-600 mb-3">{milestone.description}</p>
						<div class="text-sm text-gray-500 space-y-1">
							<p><span class="font-medium">프로젝트:</span> {getProjectName(milestone.projectId)}</p>
...
<Modal bind:open={showCreateModal} title="새 마일스톤 추가">
	<div class="space-y-4">
		<div class="grid grid-cols-2 gap-4">
			...
			<div>
				<label for="create-quarter" class="block text-sm font-medium text-gray-700 mb-1">분기 *</label>
				<input
					id="create-quarter"
					type="text"
		...
		<div>
			<label for="create-description" class="block text-sm font-medium text-gray-700 mb-1">설명</label>
			<textarea
		...
		<div class="grid grid-cols-2 gap-4">
			<div>
				<label for="create-owner" class="block text-sm font-medium text-gray-700 mb-1">담당자 *</label>
				<select
					id="create-owner"
			...
			<div>
				<label for="create-due-date" class="block text-sm font-medium text-gray-700 mb-1">마감일 *</label>
				<input
					id="create-due-date"
					type="date"
