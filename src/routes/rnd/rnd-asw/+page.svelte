  ...
  async function loadProjectData() {
    try {
      ...
      const projectResponse = await fetch(
        "/api/project-management/projects?code=RND-ASW",
      );
              ...
              <div class="border border-gray-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                  <h3 class="font-medium text-gray-900">
                    {budget.period_number}연차
                  </h3>
                  <span class="text-sm text-gray-500">
                    {formatDate(budget.start_date)} ~ {formatDate(
                      budget.end_date,
                    )}
                  </span>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <span class="ml-2 font-medium"
                      >{formatCurrency(
                        budget.personnel_cost_cash +
                          budget.personnel_cost_in_kind,
                      )}</span
                    >
                  </div>
                  <div>
                    ...
                    <span class="ml-2 font-medium"
                      >{formatCurrency(
                        budget.research_material_cost_cash +
                          budget.research_material_cost_in_kind,
                      )}</span
                    >
                  </div>
                  <div>
                    ...
                    <span class="ml-2 font-medium"
                      >{formatCurrency(
                        budget.research_activity_cost_cash +
                          budget.research_activity_cost_in_kind,
                      )}</span
                    >
                  </div>
                  <div>
                    ...
                    <span class="ml-2 font-medium"
                      >{formatCurrency(
                        budget.indirect_cost_cash +
                          budget.indirect_cost_in_kind,
                      )}</span
                    >
                  </div>
                </div>
...
{#if showEditModal}
  <ThemeModal open={showEditModal} onclose={() => (showEditModal = false)}>
    <div class="p-6 max-w-2xl">
      ...
      >
        <ThemeButton variant="ghost" onclick={() => (showEditModal = false)}
          >취소</ThemeButton
        >
  ...
  <ThemeModal open={showBudgetModal} onclose={() => (showBudgetModal = false)}>
    <div class="p-6 max-w-2xl">
      ...
      <div class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label
              for="budget-period"
              class="block text-sm font-medium text-gray-700 mb-1">연차</label
            >
            <input
              id="budget-period"
              type="number"
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label
                for="personnel-cash"
                class="block text-sm font-medium text-gray-700 mb-1">현금</label
              >
              <input
                id="personnel-cash"
                type="number"
            ...
            <div>
              <label
                for="personnel-kind"
                class="block text-sm font-medium text-gray-700 mb-1">현물</label
              >
              <input
                id="personnel-kind"
                type="number"
      ...
      <div
        class="flex justify-end space-x-3 pt-4 border-t border-gray-200 mt-6"
      >
        <ThemeButton variant="ghost" onclick={() => (showBudgetModal = false)}
          >취소</ThemeButton
        >
    ...
    <div class="p-6 max-w-lg">
      ...
      <div class="space-y-4">
        <div>
          <label
            for="member-employee"
            class="block text-sm font-medium text-gray-700 mb-1">직원</label
          >
          <select
            id="member-employee"
      ...
      <div
        class="flex justify-end space-x-3 pt-4 border-t border-gray-200 mt-6"
      >
        <ThemeButton variant="ghost" onclick={() => (showMemberModal = false)}
          >취소</ThemeButton
        >
