<script lang="ts">
	...
	function updateStatistics() {
		...
		statistics = {
			total: items.length,
			pending: items.filter(item => item.status === 'pending_approval' || item.status === 'draft').length,
			approved: items.filter(item => item.status === 'approved' || item.status === 'executed' || item.status === 'completed').length,
