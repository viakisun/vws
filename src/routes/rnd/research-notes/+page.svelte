...
<div class="container mx-auto p-6">
	...
	<div class="bg-white rounded-lg shadow-sm border p-4 mb-6">
		<div class="grid grid-cols-1 md:grid-cols-5 gap-4">
			...
			<div>
				<label for="author-filter" class="block text-sm font-medium text-gray-700 mb-1">작성자</label>
				<select
					id="author-filter"
				...
				<div class="flex justify-between items-start">
					<div class="flex-1">
						...
						<div class="text-sm text-gray-600 mb-2">
							<span class="font-medium">프로젝트:</span> {getProjectName(note.projectId)} | 
							<span class="font-medium">작성자:</span> {getPersonName(note.authorId)} | 
		...
		<div class="space-y-4">
			<div>
				...
				<div class="text-sm text-gray-600 space-y-1">
					<p><span class="font-medium">프로젝트:</span> {getProjectName(selectedNote.projectId)}</p>
					<p><span class="font-medium">작성자:</span> {getPersonName(selectedNote.authorId)}</p>
					<p><span class="font-medium">주차:</span> {selectedNote.weekOf}</p>
					<p><span class="font-medium">작성일:</span> {formatDate(selectedNote.createdAt)}</p>
					{#if selectedNote.signedAt}
						<p><span class="font-medium">서명일:</span> {formatDate(selectedNote.signedAt)}</p>
					{/if}
					{#if selectedNote.verifiedBy}
						<p><span class="font-medium">검인자:</span> {getPersonName(selectedNote.verifiedBy)}</p>
					{/if}
				</div>
			</div>
			<div>
