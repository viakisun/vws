...
export const permissionChecks = {
  ...
  canWriteAll: (roles: UserRole[]) =>
    hasPermission(Permission.WRITE_ALL, roles),
  canApproveAll: (roles: UserRole[]) =>
    hasPermission(Permission.APPROVE_ALL, roles),
  canAuditAll: (roles: UserRole[]) =>
    hasPermission(Permission.AUDIT_ALL, roles),
  canManageBudget: (roles: UserRole[]) =>
    hasPermission(Permission.MANAGE_BUDGET, roles),
  canManagePersonnel: (roles: UserRole[]) =>
    hasPermission(Permission.MANAGE_PERSONNEL, roles),
  canCreateReport: (roles: UserRole[]) =>
    hasPermission(Permission.CREATE_REPORT, roles),
  canUploadDocument: (roles: UserRole[]) =>
    hasPermission(Permission.UPLOAD_DOCUMENT, roles),
  canViewAuditLog: (roles: UserRole[]) =>
    hasPermission(Permission.VIEW_AUDIT_LOG, roles),
};
...
export const entityPermissionChecks = {
  canReadProject: (roles: UserRole[]) =>
    canPerformAction("Project", "read", roles),
  canWriteProject: (roles: UserRole[]) =>
    canPerformAction("Project", "write", roles),
  canApproveProject: (roles: UserRole[]) =>
    canPerformAction("Project", "approve", roles),
  canLockProject: (roles: UserRole[]) =>
    canPerformAction("Project", "lock", roles),

  canReadExpense: (roles: UserRole[]) =>
    canPerformAction("ExpenseItem", "read", roles),
  canWriteExpense: (roles: UserRole[]) =>
    canPerformAction("ExpenseItem", "write", roles),
  canApproveExpense: (roles: UserRole[]) =>
    canPerformAction("ExpenseItem", "approve", roles),
  canLockExpense: (roles: UserRole[]) =>
    canPerformAction("ExpenseItem", "lock", roles),

  canReadDocument: (roles: UserRole[]) =>
    canPerformAction("Document", "read", roles),
  canWriteDocument: (roles: UserRole[]) =>
    canPerformAction("Document", "write", roles),
  canApproveDocument: (roles: UserRole[]) =>
    canPerformAction("Document", "approve", roles),
  canLockDocument: (roles: UserRole[]) =>
    canPerformAction("Document", "lock", roles),

  canReadResearchNote: (roles: UserRole[]) =>
    canPerformAction("ResearchNote", "read", roles),
  canWriteResearchNote: (roles: UserRole[]) =>
    canPerformAction("ResearchNote", "write", roles),
  canApproveResearchNote: (roles: UserRole[]) =>
    canPerformAction("ResearchNote", "approve", roles),
...
export const ROLE_DESCRIPTIONS: Record<UserRole, string> = {
  [UserRole.RESEARCHER]:
    "연구원 - 연구노트 작성, 산출물 업로드, 본인 휴가/참여율 확인",
  [UserRole.PM]:
    "PM(과제책임자) - 분기 목표/산출물 정의, 참여 배정, 지출요청 승인(1차), 리스크 등록",
  [UserRole.DEPARTMENT_HEAD]:
    "담당부서(구매·기술 등) - 카테고리별 지출 집행/검수, 문서 수취",
  [UserRole.MANAGEMENT_SUPPORT]:
    "경영지원(회계·총무) - 예산 항목 관리, 증빙 검토, 내부 전자결재 기안/종결, 국가R&D 업로드 번들 생성",
  [UserRole.LAB_HEAD]:
    "연구소장 - 주간/분기 리포트 수신, 중요 승인(2차), 인력 교체 승인",
  [UserRole.EXECUTIVE]:
    "경영진 - 의사결정/에스컬레이션 승인, 신규 채용 트리거 승인",
