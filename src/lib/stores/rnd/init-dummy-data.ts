...
import { generateDummyPersons, generateDummyProjects } from './dummy-data';
import type { ExpenseItem, Document, Approval, ApprovalWorkflow } from './types';

// ===== 더미 지출 항목 생성 =====
function generateDummyExpenses(): ExpenseItem[] {
	const categories = ['PERSONNEL_CASH', 'MATERIAL', 'RESEARCH_ACTIVITY', 'TRAVEL', 'MEETING', 'PATENT', 'OFFICE_SUPPLIES'];
	...
	const requesters = ['person-002', 'person-008', 'person-009', 'person-011', 'person-013', 'person-014', 'person-015'];
	const deptOwners = ['경영지원팀', '구매팀', 'AI연구팀', '블록체인팀', 'IoT팀', '총무팀', 'R&D전략팀'];
...
function generateDummyDocuments(): Document[] {
	const documentTypes = ['REQUISITION', 'QUOTE', 'PURCHASE_ORDER', 'TAX_INVOICE', 'DELIVERY_NOTE', 'INSPECTION_REPORT', 'RECEIPT', 'MEETING_MINUTES', 'TRAVEL_REPORT', 'PATENT_DOCUMENT'];
...
export function generateStatistics() {
	...
	const stats = {
		...
		approved: expenses.filter(e => e.status === 'approved' || e.status === 'executed' || e.status === 'completed').length,
