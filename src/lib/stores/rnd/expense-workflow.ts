...
import { getDefaultWorkflow, getRequiredDocuments, checkDocumentCompleteness } from './budget-categories';
...
// ===== 필터링된 데이터 =====
export const pendingExpenses = derived(
	expenseItems,
	($expenseItems) => $expenseItems.filter(expense => 
		expense.status === 'pending_approval' || expense.status === 'draft'
	)
);

export const approvedExpenses = derived(
	expenseItems,
	($expenseItems) => $expenseItems.filter(expense => 
		expense.status === 'approved' || expense.status === 'executed' || expense.status === 'completed'
	)
);

export const rejectedExpenses = derived(
	expenseItems,
	($expenseItems) => $expenseItems.filter(expense => expense.status === 'rejected')
);
...
export function processApproval(approvalData: {
	...
	approvalWorkflows.update(workflows => {
		const workflowIndex = workflows.findIndex(w => 
			w.subjectType === approvalData.subjectType && w.subjectId === approvalData.subjectId
		);
...
export function getPendingApprovalExpenses(approverId: UUID): ExpenseItem[] {
	...
	const pendingWorkflows = workflows.filter(workflow => 
...
export function cancelExpenseItem(expenseId: UUID, reason: string): boolean {
	// 워크플로우 취소
	approvalWorkflows.update(workflows => {
		const index = workflows.findIndex(w => 
			w.subjectType === 'expense' && w.subjectId === expenseId
		);
		if (index !== -1) {
