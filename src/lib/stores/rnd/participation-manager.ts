...
export const participationRecommendations = writable<
...
export class ParticipationManager {
  ...
  static analyzeMonthlyParticipation(): Array<{
    ...
    return months.map((month) => {
      const personAnalysis = employees.map((employee) => {
        ...
        const totalRate = personAssignments.reduce(
          (sum, a) => sum + a.ratePct,
          0,
        );
  ...
  static generateOptimizationRecommendations(): ParticipationRecommendation[] {
    ...
    if (overloadConflicts.length > 0) {
      recommendations.push({
        ...
        affectedProjects: [
          ...new Set(overloadConflicts.flatMap((c) => c.affectedProjects)),
        ],
        estimatedImpact: {
          cost: 0,
    ...
    // 신규 채용 추천
    const underutilizationConflicts = conflicts.filter(
      (c) => c.conflictType === "underutilization",
    );
    ...
    if (totalUnderutilized === 0) {
      ...
      if (activeProjects.length > totalEmployees * 0.8) {
        recommendations.push({
          ...
          title: "신규 연구원 채용 고려",
          description:
            "프로젝트 수가 인력 대비 많아 신규 채용을 고려해볼 수 있습니다.",
          affectedPersons: [],
  ...
  // 5. 참여율 자동 조정
  static autoAdjustParticipation(
    personId: string,
    targetRate: number,
    reason: string,
  ): boolean {
    ...
    // 현재 총 참여율 계산
    const currentTotal = personAssignments.reduce(
      (sum, a) => sum + a.ratePct,
      0,
    );
  ...
  // 6. 참여율 업데이트 (히스토리 포함)
  static updateParticipationRate(
    assignmentId: string,
    newRate: number,
    reason: string,
  ): void {
  ...
  static simulateParticipationChanges(
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (monthData) {
        const overloadCount = monthData.personAnalysis.filter(
          (p) => p.totalRate > 100,
        ).length;
    ...
    if (monthlyImpact.some((m) => m.overloadCount > 0)) {
      recommendations.push(
        "일부 인력의 과부하가 예상됩니다. 추가 인력 투입을 고려하세요.",
      );
    }
    if (monthlyImpact.some((m) => m.underutilizedCount > 0)) {
      recommendations.push(
        "일부 인력의 미활용이 예상됩니다. 추가 프로젝트 배정을 고려하세요.",
      );
    }
...
export const participationHealth = derived(
  ...
  ([assignments, conflicts]) => {
    ...
    const healthScore = Math.max(
      0,
      100 - criticalConflicts * 20 - highConflicts * 10,
    );
...
export const participationTrends = derived(participationHistory, (history) => {
  ...
  const recentHistory = history.filter(
    (h) => new Date(h.changedAt) >= sixMonthsAgo,
  );
    ...
    averageChange:
      recentHistory.length > 0
        ? recentHistory.reduce(
            (sum, h) => sum + Math.abs(h.newRate - h.oldRate),
            0,
