...
export function payBonus(id: string) {
  bonuses.update(current =>
    current.map(bonus =>
      bonus.id === id ? { ...bonus, status: 'paid', paidAt: new Date().toISOString() } : bonus
    )
  )
}
...
export function rejectWelfareApplication(id: string, rejectionReason: string) {
  welfareApplications.update(current =>
    current.map(app => (app.id === id ? { ...app, status: 'rejected', rejectionReason } : app))
  )
}
...
export function completeWelfareApplication(id: string) {
  welfareApplications.update(current =>
    current.map(app =>
      app.id === id ? { ...app, status: 'completed', completedAt: new Date().toISOString() } : app
    )
  )
}
...
export function payEventSupport(id: string) {
  eventSupports.update(current =>
    current.map(support =>
      support.id === id ? { ...support, status: 'paid', paidAt: new Date().toISOString() } : support
    )
  )
}
...
export function calculateAnnualCompensation(
  ...
  const annualPayrolls = payrollList.filter(
    payroll => payroll.employeeId === employeeId && payroll.payPeriod.year === year
  )
