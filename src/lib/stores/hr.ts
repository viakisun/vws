...
export const employmentContracts =
  writable<EmploymentContract[]>(initialContracts);
export const jobDescriptions = writable<JobDescription[]>(
...
export function addJobDescription(
  jd: Omit<JobDescription, "id" | "createdAt" | "updatedAt">,
) {
...
// 경력 기록 관리 함수들
export function addCareerRecord(
  record: Omit<CareerRecord, "id" | "createdAt">,
) {
...
export function getEmployeeContract(
  ...
  return contractList.find(
    (contract) =>
      contract.employeeId === employeeId && contract.status === "active",
  );
}
...
export function getEmployeeAgreements(
  employeeId: string,
