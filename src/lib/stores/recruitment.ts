...
const initialJobPostings: JobPosting[] = [
	{
		...
		location: '서울 본사',
		description: 'React 기반 웹 애플리케이션 개발 및 팀 리딩을 담당할 시니어 개발자를 모집합니다.',
		requirements: [
			'5년 이상 프론트엔드 개발 경험',
			'React, TypeScript 숙련',
...
export const interviewSchedules = writable<InterviewSchedule[]>(initialInterviewSchedules);
export const interviewEvaluations = writable<InterviewEvaluation[]>(initialInterviewEvaluations);
export const offerLetters = writable<OfferLetter[]>(initialOfferLetters);

// 채용 공고 관리 함수들
export function addJobPosting(posting: Omit<JobPosting, 'id' | 'createdAt' | 'updatedAt'>) {
...
// 면접 일정 관리 함수들
export function scheduleInterview(schedule: Omit<InterviewSchedule, 'id' | 'createdAt' | 'updatedAt'>) {
...
// 면접 평가 관리 함수들
export function addInterviewEvaluation(evaluation: Omit<InterviewEvaluation, 'id' | 'evaluatedAt'>) {
...
// 오퍼 레터 관리 함수들
export function createOfferLetter(offer: Omit<OfferLetter, 'id' | 'createdAt' | 'updatedAt'>) {
...
// 유틸리티 함수들
export function getCandidatesByJobPosting(jobPostingId: string, candidateList: Candidate[]): Candidate[] {
	return candidateList.filter(candidate => candidate.jobPostingId === jobPostingId);
}

export function getCandidatesByStatus(status: Candidate['status'], candidateList: Candidate[]): Candidate[] {
	return candidateList.filter(candidate => candidate.status === status);
}

export function getInterviewSchedulesByCandidate(candidateId: string, scheduleList: InterviewSchedule[]): InterviewSchedule[] {
	return scheduleList.filter(schedule => schedule.candidateId === candidateId);
}

export function getInterviewEvaluationsByCandidate(candidateId: string, evaluationList: InterviewEvaluation[]): InterviewEvaluation[] {
	return evaluationList.filter(evaluation => evaluation.candidateId === candidateId);
}

export function getOfferLetterByCandidate(candidateId: string, offerList: OfferLetter[]): OfferLetter | undefined {
...
export function getRecruitmentStats(jobPostingId: string, candidateList: Candidate[]): {
