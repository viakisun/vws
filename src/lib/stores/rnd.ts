...
const initialProjects: Project[] = Array.from({ length: 10 }).map((_, i) => {
  ...
  const spentKRW = randInt(
    Math.floor(budgetKRW * 0.2),
    Math.floor(budgetKRW * 0.9),
  );
...
const CATS: ExpenseDocument["category"][] = [
  "인건비",
  "재료비",
  "연구활동비",
  "여비",
...
export const quarterlyPersonnelBudgets = writable<
...
export const budgetAlerts = derived(projectsStore, (arr) => {
  return arr
    .map((p) => {
      ...
      return level
        ? { projectId: p.id, name: p.name, utilization: util, level }
        : null;
    })
...
export function getQuarterSummary(quarter: string): { totalBudgetKRW: number } {
  ...
  const totalBudgetKRW = Object.values(q).reduce(
    (sum, m) => sum + (m[quarter] ?? 0),
    0,
  );
...
export const expenseHistories = writable<
  Record<
    string,
    Array<{ status: "대기" | "승인" | "반려"; reason?: string; at: string }>
  >
>({});

export function addExpenseHistory(
  id: string,
  status: "대기" | "승인" | "반려",
  reason?: string,
) {
