...
export const expectedTransactions = writable<ExpectedTransaction[]>(
  initialExpectedTransactions,
);
...
export function createFundsReport(): FundsReport {
  ...
  return {
    ...
    totalBalance: initialBankAccounts.reduce(
...
// 통장 잔고 업데이트 함수
export function updateBankAccountBalance(
  accountId: string,
  newBalance: number,
) {
...
export function exportForAIAnalysis(): {
  ...
  expectedTransactions.subscribe(
    (value) => (currentExpectedTransactions = value),
  )();

  const totalBalance = currentBankAccounts.reduce(
    (sum, account) => sum + account.balance,
    0,
  );
