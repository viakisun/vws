...
export interface OffboardingChecklistItem {
	...
	title: string;
	description: string;
	category: 'equipment-return' | 'account-cleanup' | 'knowledge-transfer' | 'documentation' | 'exit-interview';
	assignedTo: string;
	dueDate?: string;
	status: 'pending' | 'in-progress' | 'completed' | 'cancelled';
	completedBy?: string;
...
// 온보딩 템플릿 관리 함수들
export function addOnboardingTemplate(template: Omit<OnboardingTemplate, 'id' | 'createdAt' | 'updatedAt'>) {
...
export function createOnboardingProcess(
	...
	const checklistItems: OnboardingChecklistItem[] = template ? 
...
export function completeOnboardingChecklistItem(processId: string, itemId: string, completedBy: string, notes?: string) {
	onboardingProcesses.update(current =>
		current.map(process => {
			if (process.id === processId) {
				...
				return {
					...
					actualCompletionDate: allRequiredCompleted ? new Date().toISOString().split('T')[0] : undefined,
					updatedAt: new Date().toISOString()
				};
			}
			return process;
...
export function completeOffboardingChecklistItem(processId: string, itemId: string, completedBy: string, notes?: string) {
...
// 유틸리티 함수들
export function getOnboardingProcessByEmployee(employeeId: string, processList: OnboardingProcess[]): OnboardingProcess | undefined {
	return processList.find(process => process.employeeId === employeeId);
}

export function getOffboardingProcessByEmployee(employeeId: string, processList: OffboardingProcess[]): OffboardingProcess | undefined {
	return processList.find(process => process.employeeId === employeeId);
}

export function getMentorBuddyByEmployee(employeeId: string, mentorList: MentorBuddy[]): MentorBuddy | undefined {
