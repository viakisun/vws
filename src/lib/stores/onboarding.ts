...
// 스토어 생성
export const onboardingTemplates = writable<OnboardingTemplate[]>(
  initialOnboardingTemplates,
);
export const onboardingProcesses = writable<OnboardingProcess[]>(
  initialOnboardingProcesses,
);
export const offboardingProcesses = writable<OffboardingProcess[]>(
  initialOffboardingProcesses,
);
...
// 멘토/버디 관리 함수들
export function assignMentorBuddy(
  employeeId: string,
  mentorId?: string,
  buddyId?: string,
) {
...
export function getMentorBuddyByEmployee(
  ...
  return mentorList.find(
    (mentor) => mentor.employeeId === employeeId && mentor.status === "active",
  );
}
