...
export const permissionUtils = {
  ...
  canAny(permissions: Array<{ resource: string; action: string; scope?: 'own' | 'department' | 'all' }>): boolean {
    return permissions.some((perm) => this.can(perm.resource, perm.action, perm.scope));
  },

  // 모든 권한을 가지고 있는지 확인
  canAll(permissions: Array<{ resource: string; action: string; scope?: 'own' | 'department' | 'all' }>): boolean {
