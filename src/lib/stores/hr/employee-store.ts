...
// ===== 통계 정보 =====
export const employeeStatistics = derived(
	employees,
	($employees) => {
		...
		stats.recentHires = $employees.filter(emp => 
			new Date(emp.hireDate) >= thirtyDaysAgo
		).length;

		stats.recentTerminations = $employees.filter(emp => 
			emp.terminationDate && new Date(emp.terminationDate) >= thirtyDaysAgo
		).length;
...
// ===== 팀 리더 목록 =====
export const teamLeaders = derived(
	employees,
	($employees) => {
...
// ===== 계약직 직원 목록 =====
export const contractEmployees = derived(
	employees,
	($employees) => {
...
// ===== 퇴사 예정자 목록 =====
export const terminationPending = derived(
	employees,
	($employees) => {
		const now = new Date();
...
// 직원 추가
export async function addEmployee(employee: Omit<Employee, 'id' | 'createdAt' | 'updatedAt'>): Promise<boolean> {
	isLoading.set(true);
	error.set(null);
	...
	try {
		...
		if (result.success && result.data) {
			employees.update(current => 
				current.map(emp => emp.id === id ? result.data! : emp)
			);
			return true;
		} else {
