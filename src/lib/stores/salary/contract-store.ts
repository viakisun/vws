  ...
  ([$contracts, $filter]) => {
    return $contracts.filter((contract) => {
      if ($filter.employeeId && contract.employeeId !== $filter.employeeId)
        return false;
      if ($filter.department && contract.department !== $filter.department)
        return false;
      if ($filter.position && contract.position !== $filter.position)
        return false;
      if (
        $filter.contractType &&
        contract.contractType !== $filter.contractType
      )
        return false;
      if ($filter.status && contract.status !== $filter.status) return false;
      if ($filter.startDateFrom && contract.startDate < $filter.startDateFrom)
        return false;
      if ($filter.startDateTo && contract.startDate > $filter.startDateTo)
        return false;
      if ($filter.search) {
...
export async function loadContracts(
  ...
  try {
    ...
    const currentPagination = { page: 1, limit: 20 };
    ...
    const result: ApiResponse<PaginatedResponse<SalaryContract>> =
      await response.json();
...
// 직원별 급여 정보 로드
export async function loadEmployeeSalaryInfo(
  employeeId: string,
): Promise<void> {
  isLoading.set(true);
  error.set(null);
