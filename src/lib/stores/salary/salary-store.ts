...
export const salaryStatistics = derived(
  ...
  ([$payslips, $currentPeriod, $previousPeriod]) => {
    ...
    const previousPayslips = $payslips.filter(
      (p) => p.period === $previousPeriod,
    );

    const currentTotalGross = currentPayslips.reduce(
      (sum, p) => sum + (p.totalPayments || 0),
      0,
    );
    const currentTotalNet = currentPayslips.reduce(
    ...
    const previousTotalGross = previousPayslips.reduce(
      (sum, p) => sum + (p.totalPayments || 0),
      0,
    );
    const previousTotalNet = previousPayslips.reduce(
      (sum, p) => sum + (p.netSalary || 0),
      0,
    );
...
export const departmentSalaryStats = derived(payslips, ($payslips) => {
  ...
  $payslips.forEach((payslip) => {
    ...
    if (deptStats) {
      ...
      deptStats.totalGrossSalary += parseFloat(
        String(payslip.totals?.totalPayments || 0),
      );
      deptStats.totalNetSalary += parseFloat(
        String(payslip.totals?.netSalary || 0),
      );
    }
  });
  ...
  Object.keys(stats).forEach((dept) => {
    ...
    if (stat) {
      ...
      stat.averageNetSalary =
        stat.employeeCount > 0 ? stat.totalNetSalary / stat.employeeCount : 0;
    }
  });
...
export const filteredSalaryHistory = derived(
  ...
  ([$salaryHistory, $selectedEmployee]) => {
    ...
    return $salaryHistory.filter(
      (history) => history.employeeId === $selectedEmployee,
    );
  },
);
    ...
    if (result.success && result.data) {
      salaryStructures.update((current) =>
        current.map((structure) =>
          structure.id === id ? result.data! : structure,
        ),
      );
      return true;
    } else {
