...
export function getBundleStatistics(projectId: string): any {
  ...
  const generatingBundles = history.filter(
...
function calculateAverageGenerationTime(history: SubmissionBundle[]): number {
  const completedBundles = history.filter(
    (b) => b.status === "ready" || b.status === "uploaded",
  );
...
export function exportBundleData(format: "json" | "csv" | "excel"): string {
  ...
  if (format === "json") {
    ...
    const csvHeader =
      "ID,Project ID,Period,Status,File URL,Checksum,Created By,Created At\n";
