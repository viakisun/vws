...
import type { SubmissionBundle, Project, Document, ExpenseItem, Milestone, ResearchNote } from './types';
...
// 번들 콘텐츠 생성
async function generateBundleContent(bundleId: string, projectId: string, period: string): Promise<void> {
	try {
		// 1. 프로젝트 정보 수집
...
<submission_bundle>
	...
	<budget_info>
		...
		<categories>
			${budgetInfo.categoryBreakdown.map((cat: any) => `
			<category>
	...
	<personnel_info>
		...
		<participants>
			${personnelInfo.participationDetails.map((person: any) => `
			<participant>
	...
	<performance_info>
		<milestones>
			${performanceInfo.milestones.map((milestone: any) => `
			<milestone>
				<id>${milestone.id}</id>
		...
		<deliverables>
			${performanceInfo.deliverables.map((deliverable: any) => `
			<deliverable>
...
export function exportBundleData(format: 'json' | 'csv' | 'excel'): string {
	...
	if (format === 'json') {
		...
		const csvRows = allBundles.map(bundle => 
