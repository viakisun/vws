...
import type { ReplacementRecommendation, Person, Project, ParticipationAssignment, SalaryHistory } from './types';
...
export function generateReplacementRecommendation(
	...
	logAudit('create', 'replacement_recommendation', recommendationId, {
		projectId,
		departingPersonId,
		reason,
		urgency
...
function calculateCandidateScore(
	...
	// 2. 경험 수준 매칭 (20%)
	const experienceScore = calculateExperienceScore(candidate.experienceLevel, projectRequirements.experienceLevel);
	totalScore += experienceScore * 0.2;
	reasons.push(`경험 수준: ${experienceScore}점`);
	
	// 3. 가용성 (20%)
	const availabilityScore = calculateAvailabilityScore(candidate.availability, projectRequirements.participationRate);
...
// 가용성 점수 계산
function calculateAvailabilityScore(candidateAvailability: number, requiredParticipation: number): number {
...
export function approveReplacementRecommendation(
	...
	replacementRecommendations.update(list => {
		...
		logAudit('approve', 'replacement_recommendation', recommendationId, {
			approvedPersonId,
			approverId,
			comment
...
function processReplacementAssignment(
	...
	logAudit('process_replacement', 'participation_assignment', projectId, {
		departingPersonId,
		replacementPersonId,
		effectiveDate
	}, {});
...
export function rejectReplacementRecommendation(
	...
	replacementRecommendations.update(list => {
		...
		logAudit('reject', 'replacement_recommendation', recommendationId, {
			rejectorId,
			reason
		}, updatedRecommendation);
...
export function createRecruitmentRequest(
	...
	logAudit('create', 'recruitment_request', requestId, {
		projectId,
		departingPersonId,
		requirements,
		urgency
...
export function getReplacementStatistics(projectId: string): any {
	...
	const averageScore = history.length > 0 ? 
		history.reduce((sum, r) => sum + r.recommendedPersons[0]?.score || 0, 0) / history.length : 0;
	...
	return {
		...
		approvalRate: totalRecommendations > 0 ? (approvedRecommendations / totalRecommendations) * 100 : 0,
		averageScore,
