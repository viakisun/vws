...
// 알림 타입에서 심각도 결정
function getSeverityFromAlertType(
  alertType: string,
...
// SLA 알림 해결
export function resolveSlaAlert(
  alertId: string,
  resolvedBy: string,
  resolution: string,
): void {
...
// SLA 알림 에스컬레이션
export function escalateSlaAlert(
  alertId: string,
  escalatedBy: string,
  reason: string,
): void {
...
export function getSlaStatistics(period: "day" | "week" | "month"): any {
  ...
  const periodAlerts = allAlerts.filter(
    (alert) => new Date(alert.createdAt) >= periodStart,
  );
  ...
  const escalatedAlerts = periodAlerts.filter(
  ...
  return {
    ...
    trends: {
      ...
      resolutionTrend: calculateTrend(
        dayStats.resolutionRate,
        weekStats.resolutionRate,
      ),
      escalationTrend: calculateTrend(
        dayStats.escalationRate,
        weekStats.escalationRate,
      ),
    },
  };
}
...
// SLA 알림 내보내기
export function exportSlaAlerts(
