...
export function updateDocumentStatus(
  ...
  workflowStates.update((states) => {
    ...
    const docIndex = expenseState.documents.findIndex(
    ...
    if (docIndex !== -1) {
      expenseState.documents[docIndex] = {
        ...
        verifiedBy:
          status === "verified"
            ? verifiedBy
            : expenseState.documents[docIndex].verifiedBy,
      };
    }
...
export function processApprovalWorkflow(
  ...
  workflowStates.update((states) => {
    ...
    if (decision === "approved") {
      expenseState.currentStep = getNextStep(
        expenseState.currentStep,
        expenseState.workflow,
      );
