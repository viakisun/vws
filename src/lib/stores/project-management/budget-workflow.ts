...
export function generateDocumentChecklist(categoryCode: string, categoryMaster: BudgetCategoryMaster[]): Array<{
	type: string;
	required: boolean;
	templateId?: string;
	...
	workflowStates.update(states => {
		...
		if (docIndex !== -1) {
			expenseState.documents[docIndex] = {
				...expenseState.documents[docIndex],
				status,
				uploadedAt: status === 'uploaded' ? new Date().toISOString() : expenseState.documents[docIndex].uploadedAt,
...
// 필수 문서 완료 여부 체크
export function isDocumentChecklistComplete(expenseId: string, categoryCode: string, categoryMaster: BudgetCategoryMaster[]): boolean {
	const category = categoryMaster.find(c => c.categoryCode === categoryCode);
...
// SLA 기반 마감일 계산
export function calculateSlaDeadline(categoryCode: string, categoryMaster: BudgetCategoryMaster[]): Date {
	const category = categoryMaster.find(c => c.categoryCode === categoryCode);
...
// 카테고리별 기본 소유자 가져오기
export function getDefaultOwners(categoryCode: string, categoryMaster: BudgetCategoryMaster[]): {
	primary: string;
	secondary: string;
} {
	const category = categoryMaster.find(c => c.categoryCode === categoryCode);
