...
export function calculateProjectProgress(projectId: string): {
	...
	Object.keys(quarterProgress).forEach(quarter => {
		...
		quarterProgress[parseInt(quarter)] = quarterProgress[parseInt(quarter)] / quarterMilestones.length;
	});
...
// 마일스톤 리포트 생성
export function generateMilestoneReport(projectId: string, quarter?: number): {
	summary: any;
