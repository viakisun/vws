...
function checkMilestoneCompletion(milestoneId: string): void {
  milestones.update((list) => {
    ...
    milestoneDeliverables.update((deliverables) => {
      ...
      const uploadedDeliverables = milestoneDeliverables.filter(
        (d) => d.status === "uploaded",
      );
...
// 분기별 마일스톤 목록
export function getMilestonesByQuarter(
  projectId: string,
  quarter: number,
): Milestone[] {
...
export function calculateProjectProgress(projectId: string): {
  ...
  Object.keys(quarterProgress).forEach((quarter) => {
    const quarterMilestones = projectMilestones.filter(
      (m) => m.quarter === parseInt(quarter),
    );
  ...
  const overallProgress =
    totalMilestones > 0 ? totalProgress / totalMilestones : 0;
...
export function approveMilestone(
  ...
  updateMilestoneStatus(
    milestoneId,
    decision === "approved" ? "completed" : "not-started",
    comment,
  );
...
export function generateMilestoneReport(
  ...
  const summary = {
    ...
    completedMilestones: projectMilestones.filter(
      (m) => m.status === "completed",
    ).length,
    delayedMilestones: getDelayedMilestones().filter(
      (m) => m.projectId === projectId,
    ).length,
    atRiskMilestones: getAtRiskMilestones().filter(
      (m) => m.projectId === projectId,
    ).length,
