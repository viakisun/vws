        ...
        persons.subscribe((personList) => {
          ...
          assignments.forEach((assignment) => {
            ...
            while (currentDate <= endMonth) {
              ...
              // 해당 월의 급여 정보 가져오기
              const monthlySalary = getSalaryForMonth(
                person.id,
                currentDate,
                salaries,
              );
              ...
              const monthlyAllocation = {
                ...
                workingRatio,
                allocatedAmount:
                  monthlySalary * (assignment.ratePct / 100) * workingRatio,
                currency: "KRW",
                month: monthKey,
              };
...
export function getProjectMonthlyAllocations(
  ...
  monthlySalaryAllocations.subscribe((monthlyData) => {
    allocations =
      monthlyData[month]?.filter((a) => a.projectId === projectId) || [];
  })();
...
export function getProjectTotalPersonnelCost(
  ...
  monthlySalaryAllocations.subscribe((monthlyData) => {
    ...
    while (currentDate <= endDate) {
      ...
      const projectAllocations = monthAllocations.filter(
        (a) => a.projectId === projectId,
      );
      const monthTotal = projectAllocations.reduce(
...
export function getPersonMonthlyParticipation(
  ...
  monthlySalaryAllocations.subscribe((monthlyData) => {
    participations =
      monthlyData[month]?.filter((a) => a.personId === personId) || [];
  })();
...
// 개인별 총 참여율 계산 (특정 월)
export function getPersonTotalParticipationRate(
  personId: string,
  month: string,
): number {
...
export function generateMonthlyAllocationReport(month: string): string {
  let report =
    "프로젝트코드,프로젝트명,직원명,참여율,기본급여,근무비율,배분금액,통화\n";
...
export function applySalaryChange(
  ...
  logAudit(
    ...
    "person",
    personId,
    { reason, newSalary, effectiveFrom },
    {},
  );
}
  ...
  participationAssignments.update((assignments) => {
    return assignments.map((assignment) => {
      if (
        ...
        if (
          leaveStartDate <= assignmentEnd &&
          leaveEndDate >= assignmentStart
        ) {
          // 겹치는 기간이 있으면 조정 필요
