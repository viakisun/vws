...
import type { HealthIndicator, Project, Milestone, ExpenseItem, ParticipationAssignment } from './types';
...
function calculateBudgetVarianceScore(projectId: string): number {
	...
	const averageVariance = categoryVariances.reduce((sum, cat) => sum + Math.abs(cat.variance), 0) / categoryVariances.length;
...
function calculatePerformanceHealth(projectId: string): number {
	...
	const totalParticipants = Object.values(mockPerformanceData).reduce((sum, count) => sum + count, 0);
	const weightedScore = (
...
function calculateRiskHealth(projectId: string): number {
	...
	const totalScore = (technicalRiskScore * 0.3) + (scheduleRiskScore * 0.25) + 
					  (budgetRiskScore * 0.25) + (peopleRiskScore * 0.2);
	return Math.round(totalScore);
}
...
function calculateBudgetRiskScore(projectId: string): number {
	...
	const budgetAdequacy = (mockBudgetRisks.remainingBudget / mockBudgetRisks.estimatedRemainingCost) * 100;
...
function calculatePeopleRiskScore(projectId: string): number {
	...
	const keyPersonnelRisk = (mockPeopleRisks.keyPersonnelAtRisk / mockPeopleRisks.totalKeyPersonnel) * 100;
...
export function getHealthDashboardData(): any {
	...
	const averageSchedule = allIndicators.length > 0 ? 
		allIndicators.reduce((sum, h) => sum + h.schedule, 0) / allIndicators.length : 0;
	const averageBudget = allIndicators.length > 0 ? 
		allIndicators.reduce((sum, h) => sum + h.budget, 0) / allIndicators.length : 0;
	const averagePeople = allIndicators.length > 0 ? 
		allIndicators.reduce((sum, h) => sum + h.people, 0) / allIndicators.length : 0;
	const averageRisk = allIndicators.length > 0 ? 
		allIndicators.reduce((sum, h) => sum + h.risk, 0) / allIndicators.length : 0;
...
export function exportHealthIndicators(format: 'json' | 'csv' | 'excel'): string {
	...
	if (format === 'json') {
		...
		const csvRows = allIndicators.map(h => 
