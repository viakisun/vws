...
export function calculateHealthIndicator(projectId: string): HealthIndicator {
  ...
  const overallScore =
    (scheduleScore + budgetScore + peopleScore + riskScore) / 4;
...
function calculateScheduleHealth(projectId: string): number {
  ...
  const totalScore =
    milestoneScore * 0.4 +
    deliverableScore * 0.3 +
    scheduleComplianceScore * 0.3;
...
function calculateMilestoneHealth(projectId: string): number {
  ...
  const delayedMilestones = mockMilestones.filter(
    (m) => m.status === "delayed",
  ).length;
...
function calculateDeliverableHealth(projectId: string): number {
  ...
  const deliveredDeliverables = mockDeliverables.filter(
    (d) => d.status === "delivered",
  ).length;
  const overdueDeliverables = mockDeliverables.filter(
    (d) => d.status === "overdue",
  ).length;
...
function calculateBudgetHealth(projectId: string): number {
  ...
  const totalScore =
    executionScore * 0.4 + efficiencyScore * 0.3 + varianceScore * 0.3;
  return Math.round(totalScore);
}
...
function calculatePeopleHealth(projectId: string): number {
  ...
  const totalScore =
    participationScore * 0.4 + stabilityScore * 0.3 + performanceScore * 0.3;
  return Math.round(totalScore);
}
...
function calculateTechnicalRiskScore(projectId: string): number {
  ...
  mockTechnicalRisks.forEach((risk) => {
    const levelScore =
      risk.level === "low" ? 20 : risk.level === "medium" ? 50 : 80;
    const impactScore =
      risk.impact === "low" ? 20 : risk.impact === "medium" ? 50 : 80;
...
function calculateScheduleRiskScore(projectId: string): number {
  ...
  const delayRate =
    (mockScheduleRisks.delayedMilestones / mockScheduleRisks.totalMilestones) *
    100;
...
function calculateBudgetRiskScore(projectId: string): number {
  ...
  const overrunRate =
    (mockBudgetRisks.overrunCategories / mockBudgetRisks.totalCategories) * 100;
  const budgetAdequacy =
    (mockBudgetRisks.remainingBudget / mockBudgetRisks.estimatedRemainingCost) *
    100;
...
function calculatePeopleRiskScore(projectId: string): number {
  ...
  const keyPersonnelRisk =
    (mockPeopleRisks.keyPersonnelAtRisk / mockPeopleRisks.totalKeyPersonnel) *
    100;
...
export function getHealthDashboardData(): any {
  ...
  const averageSchedule =
    allIndicators.length > 0
      ? allIndicators.reduce((sum, h) => sum + h.schedule, 0) /
        allIndicators.length
      : 0;
  const averageBudget =
    allIndicators.length > 0
      ? allIndicators.reduce((sum, h) => sum + h.budget, 0) /
        allIndicators.length
      : 0;
  const averagePeople =
    allIndicators.length > 0
      ? allIndicators.reduce((sum, h) => sum + h.people, 0) /
        allIndicators.length
      : 0;
  const averageRisk =
    allIndicators.length > 0
...
// 헬스 인디케이터 내보내기
export function exportHealthIndicators(
  ...
  if (format === "json") {
    ...
    const csvHeader =
      "Project ID,Schedule,Budget,People,Risk,Overall,Last Updated\n";
