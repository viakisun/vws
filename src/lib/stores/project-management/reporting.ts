...
function collectWeeklyData(
  ...
  // 예산 집행률
  const budgetExecution = calculateBudgetExecution(
    projectId,
    weekStart,
    weekEnd,
  );
  ...
  // 연구노트 제출률
  const researchNoteSubmission = calculateResearchNoteSubmission(
    projectId,
    weekStart,
    weekEnd,
  );
...
function collectQuarterlyData(
  ...
  return {
    ...
    financialSummary: collectFinancialSummary(
      projectId,
      quarterStart,
      quarterEnd,
    ),
    personnelSummary: collectPersonnelSummary(
      projectId,
      quarterStart,
      quarterEnd,
    ),
  ...
  return {
    ...
    lastReportDate:
      projectReports.length > 0
        ? Math.max(
            ...projectReports.map((r) => new Date(r.generatedAt).getTime()),
          )
        : null,
