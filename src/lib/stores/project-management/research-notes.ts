...
// 미제출 연구노트 체크
export function getMissingResearchNotes(projectId: string, startDate: string, endDate: string): {
	missingWeeks: string[];
...
// 연구노트 제출 현황
export function getResearchNoteSubmissionStatus(projectId: string, month: string): {
	totalWeeks: number;
	...
	const { missingWeeks } = getMissingResearchNotes(projectId, startDate.toISOString(), endDate.toISOString());
...
// 연구노트 통계
export function getResearchNoteStatistics(projectId: string, period: 'month' | 'quarter' | 'year'): {
	totalNotes: number;
