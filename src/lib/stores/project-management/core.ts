...
export function addExpenseItem(expense: Omit<ExpenseItem, 'id' | 'createdAt' | 'updatedAt'>): string {
	const id = crypto.randomUUID();
	const now = new Date().toISOString();
...
export function addParticipationAssignment(assignment: Omit<ParticipationAssignment, 'id' | 'createdAt' | 'updatedAt'>): string {
	const id = crypto.randomUUID();
	const now = new Date().toISOString();
...
export function addResearchNote(note: Omit<ResearchNote, 'id' | 'createdAt' | 'updatedAt'>): string {
	const id = crypto.randomUUID();
	const now = new Date().toISOString();
...
// 감사 로그 함수
export function logAudit(action: string, entity: string, entityId: string, oldData: any, newData: any): void {
...
export function getExpenseItemsByProject(projectId: string, expenseList: ExpenseItem[]): ExpenseItem[] {
...
export function getParticipationAssignmentsByProject(projectId: string, assignmentList: ParticipationAssignment[]): ParticipationAssignment[] {
	return assignmentList.filter(a => a.projectId === projectId);
}

export function getParticipationAssignmentsByPerson(personId: string, assignmentList: ParticipationAssignment[]): ParticipationAssignment[] {
...
export function getApprovalsBySubject(subjectType: string, subjectId: string, approvalList: Approval[]): Approval[] {
	return approvalList.filter(a => a.subjectType === subjectType && a.subjectId === subjectId);
}

export function getResearchNotesByProject(projectId: string, noteList: ResearchNote[]): ResearchNote[] {
