...
const initialPerformanceReviews: PerformanceReview[] = [
  {
    ...
    improvementAreas: ["의사소통 스킬 향상", "프로젝트 관리 능력 강화"],
...
// 스토어 생성
export const performanceReviews = writable<PerformanceReview[]>(
  initialPerformanceReviews,
);
export const feedback360 = writable<Feedback360[]>(initialFeedback360);
export const competencyMatrix = writable<CompetencyMatrix[]>(
  initialCompetencyMatrix,
);
export const trainingRecords = writable<TrainingRecord[]>(
  initialTrainingRecords,
);
export const promotionRecords = writable<PromotionRecord[]>(
  initialPromotionRecords,
);
export const compensationRecords = writable<CompensationRecord[]>(
  initialCompensationRecords,
);
...
// 360도 피드백 관리 함수들
export function addFeedback360(
  feedback: Omit<Feedback360, "id" | "createdAt">,
) {
...
export function updateCompetencyMatrix(
  id: string,
  updates: Partial<CompetencyMatrix>,
) {
...
// 승진/보상 기록 관리 함수들
export function addPromotionRecord(
...
export function addCompensationRecord(
...
export function calculateOverallRating(review: PerformanceReview): number {
  ...
  const weightedSum = review.goals.reduce(
    (sum, goal) => sum + goal.rating * goal.weight,
    0,
  );
...
export function calculateAverageFeedback360(
  ...
  const totals = revieweeFeedbacks.reduce(
    (acc, feedback) => ({
