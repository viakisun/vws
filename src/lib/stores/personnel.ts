...
const NAMES = ['김','이','박','최','정','조','윤','한','임','오','강','송','허','신','문','양','배','노','유','천'];
const GIVEN = ['민수','서연','도윤','지우','서준','하은','서윤','지호','하준','예준','지민','윤서','가은','현우','유진','시우','민준','수현'];
...
const initialPersonnel: Personnel[] = Array.from({ length: 30 }).map((_, i) => {
	...
	const participations = selected.map((pid, idx) => {
		const allocation = idx === selected.length - 1 ? Math.max(10, remaining) : randInt(15, Math.max(15, remaining - 15*(selected.length-idx-1)));
		...
		return {
			...
			quarterlyBreakdown: {
				'2025-Q3': Math.round((annualSalaryKRW * (allocation/100)) / 4 + randInt(-300_000, 300_000)),
				'2025-Q4': Math.round((annualSalaryKRW * (allocation/100)) / 4 + randInt(-300_000, 300_000)),
				'2026-Q1': Math.round((annualSalaryKRW * (allocation/100)) / 4 + randInt(-300_000, 300_000)),
				'2026-Q2': Math.round((annualSalaryKRW * (allocation/100)) / 4 + randInt(-300_000, 300_000)),
			}
		};
	});
	return { id, name, role, organization, employmentType, status, annualSalaryKRW, participations } as Personnel;
});
