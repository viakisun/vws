...
const initialPersonnel: Personnel[] = Array.from({ length: 30 }).map((_, i) => {
  ...
  const status: Personnel["status"] = pick([
    "재직",
    "재직",
    "재직",
    "신규",
  ...
  const participations = selected.map((pid, idx) => {
    const allocation =
      idx === selected.length - 1
        ...
        : randInt(
            15,
            Math.max(15, remaining - 15 * (selected.length - idx - 1)),
          );
    ...
    return {
      ...
      quarterlyBreakdown: {
        "2025-Q3": Math.round(
          (annualSalaryKRW * (allocation / 100)) / 4 +
            randInt(-300_000, 300_000),
        ),
        "2025-Q4": Math.round(
          (annualSalaryKRW * (allocation / 100)) / 4 +
            randInt(-300_000, 300_000),
...
export function estimateMonthlyCostKRW(
