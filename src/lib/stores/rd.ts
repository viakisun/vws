...
const mockRecommendations: Recommendation[] = [
  {
    ...
    title: "AI 전문가 채용 필요",
    description:
      "AI 스마트팩토리 프로젝트의 진행률이 예상보다 낮습니다. 추가 AI 전문가 채용이 필요합니다.",
  ...
  },
  {
    ...
    title: "김연구 참여율 조정",
    description:
      "김연구원의 총 참여율이 80%로 높습니다. 배터리 연구 프로젝트의 참여율을 20%로 줄이는 것을 권장합니다.",
  ...
  },
  {
    ...
    description:
      "IoT 센서 네트워크 프로젝트의 재료비 예산이 부족합니다. 장비비에서 일부를 이전하는 것을 권장합니다.",
    impact: "프로젝트 지연 방지",
    estimatedCost: 0,
...
export const participations =
  writable<ProjectParticipation[]>(mockParticipations);
export const rdBudgets = writable<RDBudget[]>(mockRDBudgets);
export const documentTemplates = writable<DocumentTemplate[]>(
  mockDocumentTemplates,
);
export const documentSubmissions = writable<DocumentSubmission[]>(
  mockDocumentSubmissions,
);
...
export function getParticipationsByEmployee(
  employeeId: string,
...
export function getParticipationsByProject(
  projectId: string,
): ProjectParticipation[] {
  let participationsList: ProjectParticipation[] = [];
  participations.subscribe((parts) => {
...
export function getTotalParticipationRate(employeeId: string): number {
  ...
  participations.subscribe((parts) => {
    ...
    totalRate = employeeParts.reduce(
      (sum, part) => sum + part.participationRate,
      0,
    );
  })();
  return totalRate;
...
export function getProjectBudgetUtilization(projectId: string): number {
  ...
  rdBudgets.subscribe((budgets) => {
    ...
    const totalPlanned = projectBudgets.reduce(
      (sum, budget) => sum + budget.plannedAmount,
      0,
    );
    const totalActual = projectBudgets.reduce(
      (sum, budget) => sum + budget.actualAmount,
      0,
    );
...
export function getDocumentSubmissionStatus(projectId: string): {
  submitted: number;
  total: number;
  pending: number;
} {
  ...
  documentTemplates.subscribe((templates) => {
    documentSubmissions.subscribe((submissions) => {
      ...
      status.submitted = projectSubmissions.filter(
...
export function addParticipation(
  participation: Omit<ProjectParticipation, "id">,
): void {
...
export function updateParticipation(
  id: string,
  updates: Partial<ProjectParticipation>,
): void {
  participations.update((parts) =>
...
export function addDocumentSubmission(
...
export function updateDocumentSubmission(
  id: string,
