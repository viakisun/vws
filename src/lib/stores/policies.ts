...
export const securityPolicies = writable<SecurityPolicy[]>(
...
export const policyAgreements = writable<PolicyAgreement[]>(
  initialPolicyAgreements,
);

// 인사 규정 관리 함수들
export function addHRPolicy(
  policy: Omit<HRPolicy, "id" | "createdAt" | "updatedAt">,
) {
...
// 윤리강령 관리 함수들
export function addCodeOfConduct(
  conduct: Omit<CodeOfConduct, "id" | "createdAt" | "updatedAt">,
) {
...
export function updateCodeOfConduct(
  id: string,
  updates: Partial<CodeOfConduct>,
) {
...
export function getHRPoliciesByCategory(
  ...
  return policyList.filter(
    (policy) => policy.category === category && policy.status === "active",
  );
}
...
export function getFAQsByCategory(
  ...
  return faqList.filter(
    (faq) => faq.category === category && faq.status === "active",
  );
}
...
export function getGuidelinesByCategory(
  ...
  return guidelineList.filter(
    (guideline) =>
      guideline.category === category && guideline.status === "active",
  );
}
...
export function hasEmployeeAgreedToPolicy(
  ...
  return agreementList.some(
    (agreement) =>
      agreement.employeeId === employeeId && agreement.policyId === policyId,
  );
}
