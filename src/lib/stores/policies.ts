...
export interface HRPolicy {
	id: string;
	title: string;
	category: 'attendance' | 'leave' | 'compensation' | 'performance' | 'discipline' | 'safety' | 'general';
	content: string;
	version: string;
...
const initialFAQs: FAQ[] = [
	{
		...
		answer: '입사 후 1년이 지나면 연차휴가를 사용할 수 있습니다. 단, 입사 6개월 후부터는 월 1일씩 선사용이 가능합니다.',
		tags: ['연차', '휴가', '입사'],
		viewCount: 45,
		lastUpdated: '2023-01-01',
		status: 'active',
	...
	},
	{
		...
		answer: '연 1회 종합건강검진 비용을 지원합니다. HR팀에 신청서를 제출하시면 됩니다. 최대 50만원까지 지원 가능합니다.',
		tags: ['건강검진', '복리후생', '지원'],
		viewCount: 32,
		lastUpdated: '2023-01-01',
		status: 'active',
...
export function updateCodeOfConduct(id: string, updates: Partial<CodeOfConduct>) {
	codeOfConduct.update(current =>
		current.map(conduct => 
			conduct.id === id 
				? { ...conduct, ...updates, updatedAt: new Date().toISOString() }
				: conduct
		)
	);
}
...
// 유틸리티 함수들
export function getHRPoliciesByCategory(category: HRPolicy['category'], policyList: HRPolicy[]): HRPolicy[] {
	return policyList.filter(policy => policy.category === category && policy.status === 'active');
}

export function getSecurityPoliciesByCategory(category: SecurityPolicy['category'], policyList: SecurityPolicy[]): SecurityPolicy[] {
	return policyList.filter(policy => policy.category === category && policy.status === 'active');
}
...
export function getGuidelinesByCategory(category: Guideline['category'], guidelineList: Guideline[]): Guideline[] {
	return guidelineList.filter(guideline => guideline.category === category && guideline.status === 'active');
}

export function getPolicyAgreementsByEmployee(employeeId: string, agreementList: PolicyAgreement[]): PolicyAgreement[] {
	return agreementList.filter(agreement => agreement.employeeId === employeeId);
}

export function hasEmployeeAgreedToPolicy(employeeId: string, policyId: string, agreementList: PolicyAgreement[]): boolean {
