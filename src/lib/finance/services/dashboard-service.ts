...
export class DashboardService {
  ...
  async getDailyReports(startDate: string, endDate: string): Promise<DailyFinanceReport[]> {
    try {
      ...
      console.error('자금일보 목록 조회 실패:', error)
      throw error
    }
  }
  ...
  async getAlerts(isRead?: boolean): Promise<FinanceAlert[]> {
    try {
      ...
      console.error('자금 알림 조회 실패:', error)
      throw error
    }
  }
  ...
  // 자금 예측 조회
  async getFinancePredictions(days: number = 30): Promise<any[]> {
    try {
      ...
      console.error('자금 예측 조회 실패:', error)
      throw error
    }
  }

  // 현금흐름 분석
  async getCashFlowAnalysis(startDate: string, endDate: string): Promise<any> {
    try {
      const params = new URLSearchParams()
      params.append('startDate', startDate)
      params.append('endDate', endDate)
      ...
      console.error('현금흐름 분석 조회 실패:', error)
      throw error
    }
  }
}
