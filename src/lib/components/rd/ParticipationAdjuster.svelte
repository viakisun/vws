<script lang="ts">
  ...
  let { employee, currentParticipations, availableProjects, onSave, onCancel } = $props<{
    employee: any
  ...
  function updateParticipation(id: string, field: string, value: any) {
    adjustments = adjustments.map(adj => (adj.id === id ? { ...adj, [field]: value } : adj))
  }
...
<ThemeCard class="p-6">
  ...
  <div class="border-t pt-6" style="border-color: var(--color-border);">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div>
        <label
          for="pa-project-select"
          class="block text-sm font-medium mb-1"
          style="color: var(--color-text);">프로젝트</label
        >
        <select
      ...
      <div>
        <label
          for="start-date"
          class="block text-sm font-medium mb-1"
          style="color: var(--color-text);">시작일</label
        >
        <input
          id="start-date"
          type="date"
      ...
      <div>
        <label
          for="end-date"
          class="block text-sm font-medium mb-1"
          style="color: var(--color-text);">종료일</label
        >
        <input
          id="end-date"
          type="date"
      ...
      <div>
        <label
          ...
          class="block text-sm font-medium mb-1"
          style="color: var(--color-text);">참여율 (%)</label
        >
        <input
      ...
      <div>
        <label
          for="monthly-salary"
          class="block text-sm font-medium mb-1"
          style="color: var(--color-text);">월급여</label
        >
        <input
          id="monthly-salary"
          type="number"
      ...
      <div>
        <label for="role" class="block text-sm font-medium mb-1" style="color: var(--color-text);"
          >역할</label
        >
        <input
          id="role"
  ...
  <!-- 요약 정보 -->
  <div
    class="flex items-center justify-between p-4 rounded-lg mt-6"
    ...
    <div class="flex items-center gap-4">
      <div class="text-center">
        ...
        <p
          ...
          style="color: {isOverParticipation() ? 'var(--color-error)' : 'var(--color-primary)'};"
        >
