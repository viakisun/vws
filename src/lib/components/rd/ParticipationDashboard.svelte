<script lang="ts">
  ...
  function resolveConflict(conflictId: string, action: string) {
    participationConflicts.update(conflicts =>
      conflicts.map(c => (c.id === conflictId ? { ...c, resolvedAt: new Date().toISOString() } : c))
    )
    loadAnalysisData()
  }
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs" style="color: var(--color-text-secondary);">
            중요: {$participationConflicts.filter(c => c.severity === 'critical' && !c.resolvedAt)
              .length}개
          </p>
        </div>
                ...
                <td class="py-3 px-4">
                  <div class="flex items-center gap-1">
                    <ThemeButton
                      variant="ghost"
                      size="sm"
                      onclick={() => autoAdjustParticipation(personData.personId)}
                    >
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span class="text-sm font-medium" style="color: var(--color-text);">
                    {conflict.conflictType === 'overload'
                      ? '과부하'
                      : conflict.conflictType === 'underutilization'
                        ? '미활용'
                        : conflict.conflictType === 'skill_mismatch'
                          ? '스킬 불일치'
                          : '일정 충돌'}
                  </span>
                </div>
  ...
  <ThemeModal open={showConflictModal}>
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-end gap-2 pt-4">
        ...
        <ThemeButton
          variant="primary"
          onclick={() => selectedConflict && resolveConflict(selectedConflict.id, 'resolve')}
        >
          해결 완료
        </ThemeButton>
      </div>
  ...
  <ThemeModal open={showRecommendationModal}>
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-end gap-2 pt-4">
        ...
        <ThemeButton
          variant="primary"
          onclick={() => selectedRecommendation && executeRecommendation(selectedRecommendation.id)}
        >
          추천 실행
        </ThemeButton>
      </div>
  ...
  <ThemeModal open={showSimulationModal}>
    <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
              ...
              <div class="flex items-end gap-2">
                ...
                <ThemeButton
                  ...
                  size="sm"
                  onclick={() =>
                    (simulationChanges = simulationChanges.filter((_, i) => i !== index))}
                >
                  <XCircleIcon size={16} />
                </ThemeButton>
              </div>
      ...
      <ThemeButton
        variant="ghost"
        onclick={() =>
          (simulationChanges = [...simulationChanges, { personId: '', projectId: '', newRate: 0 }])}
      >
        변경 사항 추가
      </ThemeButton>
      ...
      <div class="flex items-center justify-end gap-2 pt-4">
        ...
        <ThemeButton variant="primary" onclick={runSimulation}>시뮬레이션 실행</ThemeButton>
      </div>
    </div>
  </ThemeModal>
{/if}
