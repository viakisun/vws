<script lang="ts">
  ...
  // 증빙 내역 관리 상태
  let showEvidenceModal = $state(false) // 증빙 추가 모달
  let showEvidenceDetailModal = $state(false) // 증빙 상세 모달
...
{#if selectedProject}
  ...
  <!-- 증빙 상세 모달 -->
  <EvidenceDetailModal
    bind:visible={showEvidenceDetailModal}
  ...
  <!-- 프로젝트 수정 모달 -->
  <ProjectEditModal
    bind:visible={showEditProjectModal}
    bind:projectForm
    {isUpdating}
      ...
      <div class="max-w-2xl">
        <div class="mb-6">
          ...
          <div class="text-sm text-gray-600 mb-4">
            연구개발비 변경 시 기존에 입력된 연구개발비 데이터에 영향을 줄 수 있습니다. 정말로
            예산을 수정하시겠습니까?
          </div>
        </div>
