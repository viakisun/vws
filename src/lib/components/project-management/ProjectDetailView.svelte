<script lang="ts">
  ...
  function getResearchMaterialCostInKind(budget: any): number {
    return getBudgetField(
      ...
      "research_material_cost_in_kind",
      0,
    );
  }
  ...
  function getResearchActivityCostInKind(budget: any): number {
    return getBudgetField(
      ...
      "research_activity_cost_in_kind",
      0,
    );
  }
  ...
  function formatPeriodTooltip(budget: any): string {
    ...
    const monthsDisplay =
      startDate && endDate
        ? `${calculatePeriodMonths(startDate, endDate)}개월`
        : "1년";
  ...
  function formatAssigneeNameFromFields(
    item: any,
    fallback: string = "미할당",
  ): string {
  ...
  let _showMemberModal = $state(false);
  ...
  async function validateEvidenceRegistration() {
    if (
      !newEvidenceForm.assigneeId ||
      !newEvidenceForm.dueDate ||
    ...
    const selectedCategory = evidenceCategories.find(
      (cat) => cat.id === newEvidenceForm.categoryId,
    );
    ...
    try {
      const response = await fetch(
        "/api/project-management/evidence-items/validate-employment",
        {
          method: "POST",
  ...
  function updateMemberValidationStatuses(issues: any[]) {
    ...
    projectMembers.forEach((member) => {
      const memberIssues = issues.filter(
        (issue) => issue.memberId === member.id,
      );
      ...
      if (memberIssues.length === 0) {
        ...
        const hasErrors = memberIssues.some(
          (issue) => issue.severity === "error",
        );
        const hasWarnings = memberIssues.some(
          (issue) => issue.severity === "warning",
        );
        const errorCount = memberIssues.filter(
          (i) => i.severity === "error",
        ).length;
        const warningCount = memberIssues.filter(
    ...
    try {
      const response = await fetch("/api/project-management/project-budgets", {
        ...
        body: JSON.stringify({
          ...
          researchMaterialCostCash: fromThousands(
            budgetForm.researchMaterialCostCash,
          ),
          researchActivityCostCash: fromThousands(
            budgetForm.researchActivityCostCash,
          ),
          ...
          researchMaterialCostInKind: fromThousands(
            budgetForm.researchMaterialCostInKind,
          ),
          researchActivityCostInKind: fromThousands(
            budgetForm.researchActivityCostInKind,
          ),
          researchStipendInKind: fromThousands(
            budgetForm.researchStipendInKind,
          ),
  ...
  async function addMember() {
    // 참여율 검증
    if (
      memberForm.participationRate < 0 ||
      memberForm.participationRate > 100
    ) {
  ...
  async function updateMember() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/project-management/project-members/${editingMember.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function removeMember(memberId: string) {
    ...
    try {
      const response = await fetch(
        `/api/project-management/project-members/${memberId}`,
        {
          method: "DELETE",
        },
      );
  ...
  async function updateBudget() {
    ...
    try {
      const response = await fetch(
        `/api/project-management/project-budgets/${editingBudget.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function removeBudget(budgetId: string) {
    ...
    try {
      const response = await fetch(
        `/api/project-management/project-budgets/${budgetId}`,
        {
          method: "DELETE",
        },
      );
  ...
  async function updateProject() {
    ...
    try {
      const response = await fetch(
        `/api/project-management/projects/${selectedProject.id}`,
        {
          method: "PUT",
          headers: {
  ...
  async function calculateMonthlyAmount(
    ...
    const rate =
      typeof participationRate === "string"
  ...
  async function openEvidenceDetail(item) {
    ...
    if (item.id) {
      try {
        const response = await fetch(
          `/api/project-management/evidence/${item.id}`,
        );
        const result = await response.json();
  ...
  async function loadEvidenceCategories() {
    try {
      const response = await fetch(
        "/api/project-management/evidence-categories",
      );
      const result = await response.json();
  ...
  async function addEvidenceItem(categoryId, itemData) {
    try {
      const currentBudget =
        projectBudgets.find(
          (b) => getPeriodNumber(b) === selectedEvidencePeriod,
        ) || projectBudgets[0];
  ...
  async function _updateEvidenceItem(itemId, updateData) {
    try {
      const response = await fetch(
        `/api/project-management/evidence/${itemId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async function handleAddEvidenceItem() {
    if (
      !newEvidenceForm.categoryId ||
      !newEvidenceForm.name ||
      !newEvidenceForm.budgetAmount
    ) {
  ...
  function calculateBudgetTotals() {
    ...
    const totals = projectBudgets.reduce(
      (acc, budget) => {
        ...
        acc.researchMaterialCash +=
          parseFloat(budget.research_material_cost_cash) || 0;
        acc.researchMaterialInKind +=
          parseFloat(budget.research_material_cost_in_kind) || 0;
        acc.researchActivityCash +=
          parseFloat(budget.research_activity_cost_cash) || 0;
        acc.researchActivityInKind +=
          parseFloat(budget.research_activity_cost_in_kind) || 0;
        acc.researchStipendCash +=
          parseFloat(budget.research_stipend_cash) || 0;
        acc.researchStipendInKind +=
          parseFloat(budget.research_stipend_in_kind) || 0;
    ...
    while (currentDate <= endDate) {
      ...
      projectMembers.forEach((member) => {
        ...
        if (memberStartDate <= monthEnd && memberEndDate >= monthStart) {
          const memberMonthlyCost =
            parseFloat(member.monthlyAmount || member.monthly_amount) || 0;
...
{#if selectedProject}
        ...
        <div class="flex-1">
          ...
          <div class="flex items-center gap-2 mb-3">
            ...
            <ThemeBadge variant="info" size="md">
              {getSponsorTypeText(
                selectedProject.sponsor_type || selectedProject.sponsorType,
              )}
            </ThemeBadge>
            <ThemeBadge variant="primary" size="md">
              {getResearchTypeText(
                selectedProject.research_type || selectedProject.researchType,
              )}
            </ThemeBadge>
          </div>
        ...
        <table class="w-full divide-y divide-gray-200" style:min-width="100%">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            {#each projectBudgets as budget, i (i)}
              ...
              {@const materialCash =
                Number(getResearchMaterialCostCash(budget)) || 0}
              {@const activityCash =
                Number(getResearchActivityCostCash(budget)) || 0}
              ...
              {@const personnelInKind =
                Number(getPersonnelCostInKind(budget)) || 0}
              {@const materialInKind =
                Number(getResearchMaterialCostInKind(budget)) || 0}
              {@const activityInKind =
                Number(getResearchActivityCostInKind(budget)) || 0}
              {@const stipendInKind =
                Number(getResearchStipendInKind(budget)) || 0}
              {@const indirectInKind =
                Number(getIndirectCostInKind(budget)) || 0}
              {@const inKindTotal =
                personnelInKind +
                materialInKind +
                activityInKind +
              ...
              <tr class="hover:bg-gray-50">
                <!-- 연차 -->
                <td
                  class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900 w-24"
                >
                  <div
                ...
                <!-- 액션 -->
                <td
                  class="px-4 py-4 whitespace-nowrap text-sm font-medium w-32"
                >
            ...
            {#if projectBudgets && projectBudgets.length > 0}
              ...
              <tr class="bg-gray-100 border-t-2 border-gray-300">
                <!-- 연차 -->
                <td
                  class="px-6 py-6 whitespace-nowrap text-sm text-gray-900 w-24"
                >
                ...
                >
                  <div class="space-y-2">
                    ...
                    <div
                      class="text-sm text-gray-800 font-medium border-t pt-2"
                    >
                      소계: {formatCurrency(
                        (totals.personnelCash || 0) +
                          (totals.personnelInKind || 0),
                        false,
                      )}
                    </div>
                  </div>
                ...
                >
                  <div class="space-y-2">
                    ...
                    >
                      소계: {formatCurrency(
                        (totals.researchMaterialCash || 0) +
                          (totals.researchMaterialInKind || 0),
                        false,
                      )}
                    </div>
                  </div>
                ...
                >
                  <div class="space-y-2">
                    ...
                    >
                      소계: {formatCurrency(
                        (totals.researchActivityCash || 0) +
                          (totals.researchActivityInKind || 0),
                        false,
                      )}
                    </div>
                  </div>
                ...
                >
                  <div class="space-y-2">
                    ...
                    >
                      소계: {formatCurrency(
                        (totals.researchStipendCash || 0) +
                          (totals.researchStipendInKind || 0),
                        false,
                      )}
                    </div>
                  </div>
                ...
                >
                  <div class="space-y-2">
                    ...
                    >
                      소계: {formatCurrency(
                        (totals.indirectCash || 0) +
                          (totals.indirectInKind || 0),
                        false,
                      )}
                    </div>
                  </div>
                ...
                >
                  <div class="space-y-2">
                    ...
                    <div
                      class="text-base text-gray-900 font-bold border-t-2 pt-2"
                    >
  ...
  {#if addingMember}
    <ThemeCard
      class="p-6 mb-6 border-green-200 bg-gradient-to-r from-green-50 to-emerald-50"
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          <label
            ...
            class="block text-sm font-medium text-gray-700 mb-2">연구원</label
          >
          <select
          ...
          >
            <option value=""
              >👥 연구원 선택 ({availableEmployees.length}명)</option
            >
        ...
        <div>
          <label
            ...
            class="block text-sm font-medium text-gray-700 mb-2">역할</label
          >
          <select
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        ...
        <div>
          <label
            ...
            class="block text-sm font-medium text-gray-700 mb-2">월간금액</label
          >
            ...
            <input
              ...
              oninput={() => {
                ...
                calculatedMonthlyAmount =
                  parseFloat(memberForm.monthlyAmount) || 0;
              }}
              ...
              {:else if calculatedMonthlyAmount > 0 && !isManualMonthlyAmount}
                <div class="flex items-center text-green-600">
                  ...
                  <span class="font-medium"
                    >자동: {formatCurrency(calculatedMonthlyAmount)}</span
                  >
                </div>
      ...
      <div class="flex justify-end space-x-3 mt-6">
        ...
        <ThemeButton
          variant="primary"
          onclick={addMember}
          disabled={!memberForm.employeeId ||
            !memberForm.startDate ||
            !memberForm.endDate}
          class="px-6 py-2"
        >
      ...
      >
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          {#each projectMembers as member, i (i)}
            <tr
              class="hover:bg-gray-50 {editingMember &&
            ...
            >
              ...
              <td
                class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 w-24"
              >
                ...
                {#if editingMember && editingMember.id === member.id}
                  <div class="space-y-2">
                    <div class="flex items-center gap-3">
                      <span class="text-xs font-medium text-blue-700 w-10"
                        >시작:</span
                      >
                      <input
                        type="date"
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                ...
                {:else}
                  <ThemeBadge
                    variant={(member.contributionType ||
                      member.contribution_type) === "cash"
                      ? "success"
                      : "warning"}
                    size="sm"
                  >
                    {(member.contributionType || member.contribution_type) ===
                    "cash"
                      ? "현금"
                      : "현물"}
                  </ThemeBadge>
                        ...
                        >
                          ...
                          {#if validationStatus.issues && validationStatus.issues.length > 0}
                            {#each validationStatus.issues as issue, i (i)}
                              <div
                                class="mb-2 last:mb-0 p-2 bg-gray-800 rounded"
                              >
                                ...
                                {#if issue.data}
                                  <div class="text-gray-500 mt-1 text-xs">
                                    ...
                                    {:else if issue.type === "duplicate_participation"}
                                      총 참여율: {issue.data.totalParticipationRate?.toFixed(
                                        1,
                                      )}%
                                      {#if issue.data.conflictingProjects && issue.data.conflictingProjects.length > 0}
                                        <br />충돌 프로젝트: {issue.data
                                          .conflictingProjects.length}개
                                      {/if}
      ...
      <div class="mt-4 p-3 bg-gray-50 rounded-lg">
        <div class="flex items-center justify-between mb-2">
          ...
          <button
            type="button"
            onclick={() =>
              (isPersonnelSummaryExpanded = !isPersonnelSummaryExpanded)}
        ...
        {#if isPersonnelSummaryExpanded}
          <div class="mt-4 pt-4 border-t border-gray-200">
            <h5 class="text-xs font-medium text-gray-600 mb-3">
              월별 인건비 상세
            </h5>
            <div class="overflow-x-auto">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  {#each personnelSummary.monthlyCosts as monthData, i (i)}
                    <tr class="hover:bg-gray-50">
                      ...
                      <td
                        class="px-3 py-2 whitespace-nowrap text-sm text-right text-green-600"
                      >
                        {formatCurrency(monthData.cash)}
                      </td>
                      <td
                        class="px-3 py-2 whitespace-nowrap text-sm text-right text-orange-600"
                      >
                  ...
                  <tr class="bg-gray-100 border-t-2 border-gray-300">
                    <td
                      class="px-3 py-2 whitespace-nowrap text-sm font-bold text-gray-900"
                    >
                      연차 합계
                    </td>
                    <td
              ...
              >
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div class="text-xs text-gray-500">
                      예산: {formatCurrency(totalAmount)} | 증빙: {totalItems}개
                      | 완료: {completedItems}개 | 진행중: {inProgressItems}개
                    </div>
                  </div>
                </div>
                <div class="flex items-center space-x-3">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                          {#each categoryItems as item, i (i)}
                            ...
                            <tr class="hover:bg-gray-50">
                              ...
                              >
                                <span class="font-medium"
                                  >{formatCurrency(item.budget_amount)}</span
                                >
                              </td>
                              ...
                              <!-- 진행률 -->
                              <td
                                class="px-3 py-3 whitespace-nowrap text-sm text-gray-900"
                              >
                                <div class="flex items-center">
                                  <div
                                    class="w-12 bg-gray-200 rounded-full h-2 mr-2"
                                  >
                                    <div
                                      class="h-2 rounded-full {item.progress >=
                                      100
                                        ? 'bg-green-600'
                                        : item.progress >= 70
                                          ? 'bg-blue-600'
                              ...
                              <!-- 마감일 -->
                              <td
                                class="px-3 py-3 whitespace-nowrap text-sm text-center"
                              >
                                <span
  ...
  {#if showEvidenceModal}
    ...
    >
      <div class="p-6 max-w-2xl">
        ...
        <div class="space-y-4">
          ...
          {#if newEvidenceForm.categoryId && evidenceCategories.find((cat) => cat.id === newEvidenceForm.categoryId)?.name === "인건비"}
            ...
            {:else if evidenceValidation}
              <div
                class="p-3 border rounded-md {evidenceValidation.validation
                  .isValid
        ...
        <!-- 액션 버튼 -->
        <div
          class="flex justify-end space-x-3 pt-4 border-t border-gray-200 mt-6"
        >
          <ThemeButton
            variant="ghost"
            onclick={() => (showEvidenceModal = false)}
          ...
          <ThemeButton
            ...
            disabled={isUpdating ||
              (evidenceValidation && !evidenceValidation.validation.isValid)}
          >
            {isUpdating ? "추가 중..." : "추가"}
          </ThemeButton>
        </div>
            ...
            <div class="grid grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <span class="font-medium ml-2"
                  >{validationResults.summary?.total || 0}</span
                >
              </div>
              <div>
                ...
                <div class="flex justify-between">
                  ...
                  <span
                    class={validationResults.schema.database?.isValid
                      ...
                      : "text-red-600"}
                  >
                    {validationResults.schema.database?.isValid
                      ? "유효"
                      : "문제 있음"}
                  </span>
                </div>
                <div class="flex justify-between">
                  ...
                  <span
                    class={validationResults.schema.naming?.isValid
                      ...
                      : "text-red-600"}
                  >
                    {validationResults.schema.naming?.isValid
                      ? "유효"
                      : "문제 있음"}
                  </span>
                </div>
              ...
              <div class="space-y-2 text-sm">
                {#each validationResults.project.validationResults || [] as result, i (i)}
                  <div class="flex justify-between">
                    ...
                    <span
                      class={result.isValid ? "text-green-600" : "text-red-600"}
                    >
