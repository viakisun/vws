<script lang="ts">
	...
	// Props
	let { projectId, members = [], onMemberUpdate }: {
		projectId: string;
		members?: any[];
	...
	function getValidationText() {
		...
		return validationState.isValid ? '검증 완료' : `${validationState.summary.invalidMembers}개 이슈 발견`;
	}
	...
	async function runValidation() {
		...
		try {
			const response = await fetch(`/api/project-management/researcher-validation?projectId=${projectId}`);
			...
			{#if validationState.summary.totalMembers > 0}
				<div class="mt-4 grid grid-cols-3 gap-4">
					<div class="text-center">
						<div class="text-2xl font-bold text-gray-900">{validationState.summary.totalMembers}</div>
						<div class="text-sm text-gray-600">전체 연구원</div>
					</div>
					<div class="text-center">
						<div class="text-2xl font-bold text-green-600">{validationState.summary.validMembers}</div>
						<div class="text-sm text-gray-600">정상</div>
					</div>
					<div class="text-center">
						<div class="text-2xl font-bold text-red-600">{validationState.summary.invalidMembers}</div>
			...
			<table class="w-full">
				<thead>
					<tr class="border-b" style="border-color: var(--color-border);">
						...
						<th class="text-left py-3 px-4 font-medium" style="color: var(--color-text);">기여 유형</th>
						<th class="text-left py-3 px-4 font-medium" style="color: var(--color-text);">검증 상태</th>
				...
				<tbody>
					{#each members as member}
						...
						<tr class="border-b hover:bg-opacity-50" style="border-color: var(--color-border);">
							...
							<!-- 참여율 -->
							<td class="py-3 px-4">
								<ThemeBadge variant={getParticipationRateColor(parseFloat(member.participation_rate) || 0) as any}>
				...
				<div class="mb-6 p-4 bg-gray-50 rounded-lg">
					...
					<div class="grid grid-cols-2 gap-4 text-sm">
						<div>
							...
							<span class="ml-2">{selectedMember.employee_department} / {selectedMember.employee_position}</span>
						</div>
						<div>
						...
						<div>
							...
							<span class="ml-2">{formatCurrency(parseFloat(selectedMember.monthly_amount) || 0)}</span>
						</div>
						<div>
							<span class="text-gray-600">참여기간:</span>
							<span class="ml-2">{formatDate(selectedMember.start_date)} ~ {formatDate(selectedMember.end_date)}</span>
						</div>
					</div>
				</div>

				<!-- 검증 이슈 -->
				{@const memberIssues = validationState.issues.filter(issue => issue.memberId === selectedMember.id)}
				
				{#if memberIssues.length > 0}
					<div class="space-y-3">
						<h4 class="font-medium text-gray-900">발견된 이슈</h4>
						{#each memberIssues as issue}
							<div class="p-3 rounded-lg border {issue.severity === 'error' ? 'border-red-200 bg-red-50' : 'border-yellow-200 bg-yellow-50'}">
