<script lang="ts">
	...
	// Props
	let { projectId = '', existingBudgets = [], readonly = false } = $props<{
		projectId?: string;
	...
	function calculateBudgetSummary(budgets: AnnualBudgetFormData[]): BudgetSummary {
		...
		return {
			...
			companyBurdenRatio: totalBudget > 0 ? ((totalCompanyCash + totalCompanyInKind) / totalBudget) * 100 : 0,
	...
	function validateForm(): boolean {
		...
		budgetData.forEach((budget, index) => {
			...
			const yearTotal = (budget.governmentFunding || 0) + (budget.companyCash || 0) + (budget.companyInKind || 0)
			if (yearTotal === 0) {
		...
		<div class="mb-8 p-4 bg-blue-50 rounded-lg">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
				<div>
					...
					<div class="text-xl font-bold text-blue-900">{formatCurrency(budgetSummary.totalBudget)}원</div>
				</div>
				<div>
					...
					<div class="text-lg font-semibold text-green-800">
						...
						<span class="text-sm text-green-600">({budgetSummary.governmentFundingRatio.toFixed(1)}%)</span>
					</div>
				</div>
				<div>
			...
			<div class="mt-4 pt-4 border-t border-blue-200">
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
					<div>
						...
						<span class="ml-2 font-semibold">{formatCurrency(budgetSummary.totalCash)}원 ({budgetSummary.cashRatio.toFixed(1)}%)</span>
					</div>
					<div>
						<span class="text-blue-700 font-medium">현물 총액:</span>
						<span class="ml-2 font-semibold">{formatCurrency(budgetSummary.totalInKind)}원 ({budgetSummary.inKindRatio.toFixed(1)}%)</span>
					</div>
				</div>
			</div>
		</div>
					...
					<div class="bg-gray-50 p-3 rounded-md mb-4">
						<div class="text-sm text-gray-600 space-y-1">
							...
							<div class="flex justify-between border-t pt-1 font-semibold text-gray-900">
								...
								<span>
									{formatCurrency((budget.governmentFunding || 0) + (budget.companyCash || 0) + (budget.companyInKind || 0))}원
								</span>
							</div>
						</div>
					</div>
