<script lang="ts">
  ...
  function getAnimatedSegment(segment: (typeof segments)[0]) {
    const animatedAngle =
      segment.startAngle + (segment.endAngle - segment.startAngle) * animationProgress
...
<div class="pie-chart-container" style="width: {width}px; height: {height}px;">
  <svg bind:this={svgElement} {width} {height} viewBox="0 0 {width} {height}" class="pie-chart">
    <!-- Segments -->
    {#if segments.length > 0}
      <g bind:this={pathsGroup} class="segments">
        {#each segments as segment, i}
          ...
          >
            <title
              >{segment.label}: {formatValue(segment.value)} ({segment.percentage.toFixed(
                1
              )}%)</title
            >
          </path>
