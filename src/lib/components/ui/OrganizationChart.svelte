<script lang="ts">
	...
	function isTeamLead(employee: Employee): boolean {
		return employee.job_title === 'Team Lead' || employee.position === 'Team Lead' || (employee.isTeamLead ?? false);
	}
									...
									{#each sortEmployees(department.children) as employee}
										<div class="flex items-center gap-2 text-xs">
											...
											{#if isTeamLead(employee)}
												<span class="bg-yellow-200 text-yellow-800 px-1 py-0.5 rounded text-xs">TL</span>
											{/if}
										</div>
									{/each}
								</div>
