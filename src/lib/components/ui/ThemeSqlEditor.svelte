<script lang="ts">
	...
	function validateSql(sqlString: string): boolean {
		try {
			...
			const sqlKeywords = ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'CREATE', 'DROP', 'ALTER', 'FROM', 'WHERE', 'JOIN', 'INNER', 'LEFT', 'RIGHT', 'OUTER', 'ON', 'GROUP', 'BY', 'ORDER', 'HAVING', 'LIMIT', 'OFFSET'];
	...
	function formatSql(sqlString: string): string {
		try {
			...
			formatted = formatted
				.replace(/\b(SELECT|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER|FROM|WHERE|JOIN|INNER|LEFT|RIGHT|OUTER|ON|GROUP|BY|ORDER|HAVING|LIMIT|OFFSET)\b/gi, '\n$1')
				.replace(/\n\s+/g, '\n')
				.trim();
			...
			for (const line of lines) {
				...
				if (trimmed.match(/^\b(WHERE|JOIN|INNER|LEFT|RIGHT|OUTER|ON|GROUP|BY|ORDER|HAVING|LIMIT|OFFSET)\b/i)) {
...
<div class={getSqlEditorClasses()} {...restProps}>
	<div class="theme-sqleditor-header">
		<div class="theme-sqleditor-title">
			SQL Editor
		</div>
