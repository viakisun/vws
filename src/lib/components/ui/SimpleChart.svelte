	...
	<div class="chart-container flex items-center justify-center" style="height: {height}px;">
		<div class="relative w-32 h-32">
			<svg class="w-32 h-32 transform -rotate-90" viewBox="0 0 32 32">
				{#each data as item, index}
					{@const startAngle = (data.slice(0, index).reduce((sum, d) => sum + d.value, 0) / totalValue) * 360}
					{@const endAngle = ((data.slice(0, index + 1).reduce((sum, d) => sum + d.value, 0)) / totalValue) * 360}
