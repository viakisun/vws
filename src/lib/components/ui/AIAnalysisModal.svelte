...
{#if isOpen}
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    ></div>
    <div
      class="relative w-full max-w-6xl mx-4 rounded-xl bg-white shadow-lg border border-gray-200"
      role="dialog"
      aria-modal="true"
    >
              ...
              <div class="bg-white border rounded-lg p-4">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p
                      class="text-2xl font-bold {getHealthColor(
                  ...
                  <div>
                    ...
                    <div class="mt-1">
                      <Badge variant={getRiskLevelColor(analysisResult.metrics.riskLevel)}>
                        {analysisResult.metrics.riskLevel === 'low'
                          ? '낮음'
                          : analysisResult.metrics.riskLevel === 'medium'
                            ? '보통'
                            : '높음'}
                      </Badge>
                    </div>
              ...
              <ul class="space-y-2">
                {#each analysisResult.insights as insight}
                  <li class="flex items-start">
                    <svg
                      class="h-5 w-5 text-blue-500 mt-0.5 mr-2 flex-shrink-0"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <div class="space-y-2">
                      <div class="flex justify-between">
                        ...
                        <span class="font-medium"
                          >₩{analysisResult.forecast.nextMonth.expectedIncome.toLocaleString()}</span
                        >
                      </div>
                      <div class="flex justify-between">
                        <span class="text-gray-600">예상 지출:</span>
                        <span class="font-medium"
                          >₩{analysisResult.forecast.nextMonth.expectedExpense.toLocaleString()}</span
                        >
                      </div>
                      <div class="flex justify-between border-t pt-2">
                        <span class="text-gray-600">순현금흐름:</span>
                        <span
                          class="font-medium {analysisResult.forecast.nextMonth.netCashFlow >= 0
                            ? 'text-green-600'
                            : 'text-red-600'}"
                        >
                  ...
                  <div>
                    ...
                    <div class="space-y-2">
                      <div class="flex justify-between">
                        <span class="text-gray-600">예상 수입:</span>
                        <span class="font-medium"
                          >₩{analysisResult.forecast.nextQuarter.expectedIncome.toLocaleString()}</span
                        >
                      </div>
                      <div class="flex justify-between">
                        <span class="text-gray-600">예상 지출:</span>
                        <span class="font-medium"
                          >₩{analysisResult.forecast.nextQuarter.expectedExpense.toLocaleString()}</span
                        >
                      </div>
                      <div class="flex justify-between border-t pt-2">
                        <span class="text-gray-600">순현금흐름:</span>
                        <span
                          class="font-medium {analysisResult.forecast.nextQuarter.netCashFlow >= 0
                            ? 'text-green-600'
                            : 'text-red-600'}"
                        >
