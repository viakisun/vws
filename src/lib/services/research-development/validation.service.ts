...
export async function validateEvidenceRegistration(
  ...
  const response = await fetch('/api/research-development/evidence-items/validate-employment', {
    method: 'POST',
...
export async function comprehensiveValidation(projectId: string): Promise<ValidationResult> {
  const response = await fetch(`/api/research-development/comprehensive-validation`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
