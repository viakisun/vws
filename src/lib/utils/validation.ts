...
export class ValidationUtils {
  ...
  static createOverallValidation(validationResults: unknown[]) {
    const validItems = validationResults.filter(
  ...
  static createErrorResponse(
    error: any,
    message: string = "검증 중 오류가 발생했습니다.",
  ) {
    return {
      success: false,
      error: message,
...
export class PersonnelCostValidator {
  ...
  static calculateActualPersonnelCost(members: unknown[], budget: any): number {
    ...
    relevantMembers.forEach((member) => {
      ...
      const actualStartDate =
        memberStartDate > budgetStartDate ? memberStartDate : budgetStartDate;
      const actualEndDate =
        memberEndDate < budgetEndDate ? memberEndDate : budgetEndDate;

      const months = ValidationUtils.getMonthsDifference(
        actualStartDate,
        actualEndDate,
      );
   ...
   * 인건비 검증
   */
  static validatePersonnelCost(
    budget: any,
    actualCost: number,
  ): ValidationResult {
    const budgetedCost = parseFloat(budget.personnel_cost) || 0;
    const isWithinTolerance = ValidationUtils.isAmountWithinTolerance(
      budgetedCost,
      actualCost,
    );
    ...
    return ValidationUtils.createValidationResult(
      ...
      [
        `예산: ${budgetedCost.toLocaleString()}원`,
        `실제: ${actualCost.toLocaleString()}원`,
      ],
      {
        budgetedCost,
        actualCost,
...
export class EmploymentPeriodValidator {
  ...
  static validateEvidenceEmploymentPeriod(
    ...
    if (hireDate && dueDate < hireDate) {
      issues.push(
        `입사일(${hireDate.toLocaleDateString()}) 이전에 인건비가 집행되었습니다.`,
      );
    }
...
export class BudgetConsistencyValidator {
  ...
  static validateBudgetConsistency(
    ...
    if (
      ValidationUtils.isAmountWithinTolerance(
        projectTotalBudget,
