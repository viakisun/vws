...
function parseTransactions(rawData: any[][]): NonghyupTransaction[] {
  ...
  for (let i = 0; i < Math.min(15, rawData.length); i++) {
    ...
    if (row) {
      ...
      if (row.some(cell => String(cell).includes('거래일자')) && 
  ...
  for (let i = headerRowIndex + 1; i < rawData.length; i++) {
    ...
    // 빈 행 또는 합계 행 건너뛰기
    if (!row || row.length === 0 || String(row[0]).includes('합') || String(row[0]).includes('소계')) {
      continue
    }
...
export function parseNonghyupBankStatement(content: string): BankStatementParseResult {
  ...
  try {
    ...
    for (const tx of excelTransactions) {
      try {
        // 날짜 검증 및 변환
        if (!tx.transactionDate || tx.transactionDate.trim() === '' || tx.transactionDate === 'undefined') {
        ...
        try {
          ...
          if (tx.transactionTime && tx.transactionTime.trim() !== '') {
            ...
            console.log(`🔥 농협 날짜+시간 조합: "${tx.transactionDate}" + "${tx.transactionTime}" = "${dateTimeStr}"`)
          } else {
            // 시간이 없으면 자정으로 설정
          ...
          console.warn(`🔥 농협 거래 건너뛰기: 날짜 변환 실패 (원본: "${tx.transactionDate}", 오류: ${error})`)
        ...
        if (tx.depositAmount > 0) {
          ...
          categoryCode = '임대료'
        } else if (tx.description.includes('전기') || tx.description.includes('가스') || tx.description.includes('수도')) {
          categoryCode = '공과금'
