 ...
 */
function parseJeonbukBankExcel(fileContent: string): JeonbukTransaction[] {
  try {
    ...
    console.log('🔥 전북은행 워크북 시트 이름들:', workbook.SheetNames)
 ...
 * 전북은행 거래내역 파싱 (엑셀 파일)
 */
export function parseJeonbukBankStatement(content: string): BankStatementParseResult {
  ...
  try {
    ...
    // 엑셀 파싱
    const excelTransactions = parseJeonbukBankExcel(content)
