...
export const EXCEL_SECURITY_CONFIG = {
  ...
  SECURITY_LEVEL:
    process.env.EXCEL_SECURITY_LEVEL || ('strict' as 'strict' | 'moderate' | 'lenient'),
}
...
export function validateExcelMagicBytes(buffer: Buffer): boolean {
  ...
  if (buffer.length >= 8) {
    if (
      buffer.subarray(0, 8).equals(xlsSignature1) ||
      buffer.subarray(0, 8).equals(xlsSignature2)
    ) {
      return true
    }
  }
...
export function validateExcelDataSize(data: any[][]): boolean {
  ...
  return rowCount <= EXCEL_SECURITY_CONFIG.MAX_ROWS && maxColCount <= EXCEL_SECURITY_CONFIG.MAX_COLS
}
  ...
  for (const row of data) {
    ...
    for (const cell of row) {
      if (typeof cell === 'string') {
        ...
        if (
          macroPatterns.some((pattern) => {
            if (pattern.endsWith(' ')) {
              // 공백으로 끝나는 패턴은 단어 경계 확인
              return (
                upperCell.includes(pattern) &&
...
export function validateExcelSecurity(
  ...
  if (!validateFileSize(buffer)) {
    errors.push(
      `파일 크기가 너무 큽니다 (${buffer.length} bytes > ${EXCEL_SECURITY_CONFIG.MAX_FILE_SIZE} bytes)`,
    )
  }
  ...
  if (data && !validateExcelDataSize(data)) {
    errors.push(
      `데이터 크기가 너무 큽니다 (${data.length}행 > ${EXCEL_SECURITY_CONFIG.MAX_ROWS}행)`,
    )
  }
