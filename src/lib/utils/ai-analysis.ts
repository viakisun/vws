...
## 통장 잔고 현황
${bankAccounts
  .map(account => `- ${account.name} (${account.bankName}): ${account.balance.toLocaleString()}원`)
  .join('\n')}
...
export function predictCashFlow(
  ...
  for (let i = 0; i < months; i++) {
    ...
    // 해당 월의 예상 거래 필터링
    const monthTransactions = expectedTransactions.filter(
      t => t.date.startsWith(monthKey) && t.status !== 'cancelled'
    )
...
export function assessRisk(
  ...
  const monthlyExpense =
    transactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0) / 12 // 월평균
  ...
  // 예상 거래의 불확실성
  const uncertainTransactions = expectedTransactions.filter(
    t => t.status === 'pending' && (t.probability || 100) < 80
  )
