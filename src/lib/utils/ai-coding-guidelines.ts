...
export const AI_CODING_GUIDELINES: CodingGuideline[] = [
	...
	},
	{
		...
		category: 'SQL',
		title: 'SQL 쿼리 포맷팅',
		description: 'SQL 쿼리는 가독성을 위해 적절히 들여쓰기하고 줄바꿈해야 합니다.',
		examples: {
			good: [
				`SELECT 
					p.id,
	...
	},
	{
		...
		title: '에러 처리 규칙',
		description: '모든 API 호출과 데이터베이스 작업에는 적절한 에러 처리가 필요합니다.',
		examples: {
			good: [
				`try {
...
export class AICodingValidator {
	...
	static validateColumnName(columnName: string): { isValid: boolean; issues: string[] } {
		...
		// 4. 패턴 검증
		const columnRule = VALIDATION_RULES.find(rule => rule.message.includes('칼럼명'))
