...
export class SchemaValidator {
    ...
    try {
      ...
      for (const rule of this.PROJECT_SCHEMA_RULES) {
        const actualColumn = actualSchema.find(
          (col) =>
            col.table_name === rule.tableName &&
            col.column_name === rule.columnName,
        );
        ...
        if (!actualColumn) {
          ...
          results.push({
            ...
            issues: [
              `칼럼 '${rule.columnName}'이 테이블 '${rule.tableName}'에 존재하지 않습니다.`,
            ],
            message: `필수 칼럼 누락: ${rule.tableName}.${rule.columnName}`,
          });
        } else {
          // 칼럼이 존재하는 경우 타입 검증
          const typeMatch = this.compareDataTypes(
            rule.expectedType,
   ...
   * 칼럼명 일관성 검증
   */
  static async validateColumnNamingConsistency(): Promise<
  ...
  > {
    ...
    try {
      ...
      for (const column of actualSchema) {
        ...
        if (
          column.column_name.includes("-") ||
          /[A-Z]/.test(column.column_name)
        ) {
          issues.push(
            "칼럼명은 언더스코어(_)를 사용해야 합니다. (camelCase 금지)",
          );
        }
        ...
        if (column.column_name.endsWith("_id") && column.data_type === "uuid") {
          ...
          if (!expectedTableName || expectedTableName.length < 2) {
            issues.push(
              "외래키 칼럼명은 의미있는 테이블명_id 형태여야 합니다.",
            );
          }
        }
  ...
  static validateQueryColumns(
    ...
    try {
      ...
      const tableRules = this.PROJECT_SCHEMA_RULES.filter(
        (rule) => rule.tableName === tableName,
      );
      ...
      for (const columnName of columnMatches) {
        ...
        if (!rule) {
          results.push({
            ...
            issues: [
              `칼럼 '${columnName}'이 테이블 '${tableName}'에 정의되지 않았습니다.`,
            ],
      ...
      return [
        {
          ...
          actualType: "error",
          issues: [
            `쿼리 파싱 오류: ${error instanceof Error ? error.message : "Unknown error"}`,
          ],
          message: "쿼리 파싱 실패",
        },
      ];
  ...
  static getTableSchemaRules(tableName: string): ColumnRule[] {
    return this.PROJECT_SCHEMA_RULES.filter(
      (rule) => rule.tableName === tableName,
    );
  }
}
