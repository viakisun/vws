...
export function validateBackup(data: any): data is BackupData {
	...
	return requiredFields.every(field => Array.isArray(data.data[field]) || typeof data.data[field] === 'object');
}
...
export function getBackupStats(
	...
	const totalRecords = bankAccounts.length + transactions.length + expectedTransactions.length + 
						 budgetCategories.length + budgetGoals.length;
	
	const backupData = createBackup(
		bankAccounts, transactions, expectedTransactions, 
		budgetCategories, budgetGoals, [], {} as NotificationSettings
	);
