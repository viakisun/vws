    ...
    if (date instanceof Date) {
      ...
      if (dateStr.includes("T")) {
        ...
        if (!isNaN(tempDate.getTime())) {
          ...
          dateObj = new Date(
            `${year}-${month}-${day}T00:00:00${timezoneOffset}`,
          );
        } else {
          dateObj = tempDate;
        }
...
export function formatDateForDisplay(
  ...
  try {
    ...
    const localDate = new Date(
      date.toLocaleString("en-US", { timeZone: currentTimezone }),
    );
...
export function formatDateTimeForInput(utcDate: StandardDate | string): string {
  ...
  try {
    ...
    logger.error(
      "DateTime input formatting error:",
      error,
      "for date:",
      utcDate,
    );
...
export function getCurrentSeoulAsUTC(): StandardDate {
  ...
  const seoulTime = new Date(
    now.toLocaleString("en-US", { timeZone: SEOUL_TIMEZONE }),
  );
 ...
 */
export function enforceStandardDate(
  ...
  if (process.env.NODE_ENV === "development") {
    if (!isValidDate(date)) {
      logger.warn(
        `⚠️ [날짜 처리 강제] ${context}에서 유효하지 않은 날짜 발견: "${date}"`,
      );
    }
  }
