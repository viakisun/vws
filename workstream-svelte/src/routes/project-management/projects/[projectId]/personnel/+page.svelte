<script lang="ts">
	...
	function quarterCost(person: Personnel): number {
		const breakdown = person.participations.find((x) => x.projectId === projectId)?.quarterlyBreakdown?.[quarter] ?? 0;
					...
					<tbody class="divide-y">
						{#each people as p}
							...
							<tr>
								...
								<td class="px-3 py-2">
									{#if qb > 0}
										<Badge color={ratio >= 100 ? 'red' : ratio >= 95 ? 'yellow' : ratio >= 80 ? 'yellow' : 'green'}>{ratio}%</Badge>
									{:else}
										-
									{/if}
								</td>
