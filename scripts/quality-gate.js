...
class QualityGate {
    ...
    async checkTypeScript() {
        ...
        try {
            ...
            if (this.results.typescript.errors > QUALITY_GATES.MAX_TYPESCRIPT_ERRORS) {
                logError(`TypeScript 오류 ${this.results.typescript.errors}개 (허용: ${QUALITY_GATES.MAX_TYPESCRIPT_ERRORS}개)`);
        ...
        try {
            ...
            try {
                ...
                if (totalWarnings > QUALITY_GATES.MAX_ESLINT_WARNINGS) {
                    logWarning(`ESLint 경고 ${totalWarnings}개 (허용: ${QUALITY_GATES.MAX_ESLINT_WARNINGS}개)`);
                    this.results.eslint.passed = false;
                    return false;
                }
    ...
    async checkSecurity() {
        ...
        try {
            ...
            if (totalVulnerabilities > QUALITY_GATES.MAX_SECURITY_VULNERABILITIES) {
                logError(`보안 취약점 ${totalVulnerabilities}개 발견 (허용: ${QUALITY_GATES.MAX_SECURITY_VULNERABILITIES}개)`);
    ...
    async checkFileSizes() {
        ...
        try {
            ...
            if (totalLines > QUALITY_GATES.MAX_FILE_SIZE * 100) { // 전체 프로젝트 기준
                logWarning(`전체 코드 라인 수 ${totalLines}줄 (권장: ${QUALITY_GATES.MAX_FILE_SIZE * 100}줄 이하)`);
