...
for (const file of files) {
  try {
    ...
    const labelInputPattern = /<label([^>]*)class="([^"]*)"([^>]*)>([^<]*)<\/label>\s*<input\s*([^>]*?)>/g;
    
    content = content.replace(labelInputPattern, (match, before, className, after, labelText, inputAttrs) => {
      // 고유한 ID 생성
    ...
    const labelSelectPattern = /<label([^>]*)class="([^"]*)"([^>]*)>([^<]*)<\/label>\s*<select\s*([^>]*?)>/g;
    
    content = content.replace(labelSelectPattern, (match, before, className, after, labelText, selectAttrs) => {
      // 고유한 ID 생성
