...
// 마이그레이션 파일 경로 (사용하지 않음)
// const _MIGRATION_FILE = join(process.cwd(), 'src/lib/database/migrations/001-convert-date-to-timestamp.sql')
...
async function runMigration() {
  ...
  try {
    ...
    const latestMigration =
      migrationFiles.find((file) => file.includes('002-simple-date-to-timestamp')) ||
      migrationFiles[0]
    ...
    if (backupCheck.rows.length > 0) {
      log(
        `⚠️  기존 백업 테이블 발견: ${backupCheck.rows.map((r) => r.table_name).join(', ')}`,
        'yellow',
      )
    ...
    // 성공 통계
    const successCount = verification.rows.filter(
