...
async function main() {
    ...
    for (const employee of employees) {
        ...
        if (!jobTitleName) {
            console.log(`⚠️  매핑되지 않은 직급: ${employee.first_name} ${employee.last_name} - ${position}`);
            skippedCount++;
            continue;
        }
        ...
        if (employee.job_title_id === jobTitleId) {
            console.log(`✅ 이미 올바른 직급: ${employee.first_name} ${employee.last_name} - ${position} → ${jobTitleName}`);
            continue;
        }
        
        console.log(`🔄 업데이트 중: ${employee.first_name} ${employee.last_name} - ${position} → ${jobTitleName}`);
