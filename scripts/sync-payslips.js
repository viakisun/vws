...
class PayslipsSync {
	...
	async getMissingPayslips() {
		...
		const missingPayslips = awsPayslips.filter(p => 
			!localPayslips.has(`${p.employee_name}-${p.period}`)
		)
	...
	async insertPayslip(payslip) {
		try {
			...
			console.error(`❌ Failed to insert payslip ${payslip.employee_name}-${payslip.period}:`, error.message)
			return false
		}
	}
	...
	async syncPayslips() {
		...
		missingPayslips.forEach((payslip, index) => {
			console.log(`   ${index + 1}. ${payslip.employee_name || 'Unknown'} - ${payslip.period} (${payslip.total_amount?.toLocaleString()}원)`)
		})
