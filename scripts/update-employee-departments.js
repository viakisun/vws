...
async function main() {
  ...
  console.log(
    `📊 총 ${employees.length}명의 직원 중 부서가 없는 직원: ${employeesWithoutDepartment.length}명`
  )
  ...
  for (const employee of employeesWithoutDepartment) {
    ...
    if (!newDepartment) {
      console.log(
        `⚠️  매핑되지 않은 직급/직책: ${employee.first_name} ${employee.last_name} - ${position} (${jobTitleName})`
      )
      skippedCount++
      continue
    }
    ...
    console.log(
      `🔄 업데이트 중: ${employee.first_name} ${employee.last_name} - ${position} → ${newDepartment}`
    )
