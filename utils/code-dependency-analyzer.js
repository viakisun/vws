...
class CodeDependencyAnalyzer {
  ...
  static async analyzeProjectDependencies() {
    ...
    // 3단계: 변경 영향도 분석
    for (const [filePath, fileAnalysis] of analysis) {
      fileAnalysis.changeImpact = this.calculateChangeImpact(
        filePath,
        fileAnalysis,
        analysis,
      );
  ...
  static generateSafeChangeProcedure(filePath, changeType, analysis) {
    ...
    const affectedFiles = [
      ...new Set(impacts.map((impact) => impact.affectedFile)),
    ];
    const procedure = [];
    const risks = [];
    ...
    // 위험 요소 식별
    if (
      fileAnalysis.riskLevel === "critical" ||
      ...
      const scanDirectory = (dir) => {
        ...
        for (const item of items) {
          ...
          try {
            ...
            if (stat.isDirectory()) {
              if (
                !this.IGNORE_PATTERNS.some((pattern) => item.includes(pattern))
              ) {
  ...
  static calculateRiskLevel(analysis) {
    ...
    // 글로벌 유틸리티 파일인지 확인
    if (
      analysis.filePath.includes("/utils/") ||
      analysis.filePath.includes("/lib/")
    ) {
      riskScore += 5;
    }
    // API 파일인지 확인
...
CodeDependencyAnalyzer.SUPPORTED_EXTENSIONS = [
  ".ts",
  ".js",
  ".svelte",
  ".vue",
...
CodeDependencyAnalyzer.IGNORE_PATTERNS = [
  "node_modules",
  ".git",
  "dist",
  "build",
