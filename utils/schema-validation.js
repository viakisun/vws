...
class SchemaValidator {
    ...
    static async validateDatabaseSchema() {
        ...
        try {
            ...
            for (const rule of this.PROJECT_SCHEMA_RULES) {
                const actualColumn = actualSchema.find(col => col.table_name === rule.tableName && col.column_name === rule.columnName);
                if (!actualColumn) {
                    // 칼럼이 존재하지 않는 경우
                    results.push({
                        isValid: false,
    ...
    static validateQueryColumns(query, tableName) {
        ...
        try {
            ...
            for (const columnName of columnMatches) {
                ...
                if (sqlKeywords.includes(columnName.toUpperCase()))
                    continue;
                // 테이블 규칙에서 해당 칼럼 찾기
    ...
    static addSchemaRule(rule) {
        const existingIndex = this.PROJECT_SCHEMA_RULES.findIndex(r => r.tableName === rule.tableName && r.columnName === rule.columnName);
        if (existingIndex >= 0) {
